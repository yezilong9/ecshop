...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'send_mobil_message', 'is_registered_ccx' );
...
//    $smarty->assign('back_act', $back_act);
    ...
    /*ccx 2014-12-20 开始 从手机端的圣诞活动过来的页面,到时候需要返回相应的页面*/
    $activity = isset($_GET['activity']) ? $_GET['activity'] : '';
    $url_request = isset($_GET['url_request']) ? $_GET['url_request'] : '';
    if($activity == 'christmas_activity')
    {
       //$smarty->assign('back_act', 'christmas_activity.php'); 
         $smarty->assign('back_act', $url_request); 
    }
    elseif($activity == 'login_draw_activity')
    {
       //$smarty->assign('back_act', 'http://www.com/mobile/draw/201501/');
         $smarty->assign('back_act', $url_request); 
    }
    /*ccx 2014-12-20 结束 从手机端的圣诞活动过来的页面,到时候需要返回相应的页面*/
...
elseif ($action == 'act_register')
{
    ...
    else
    {
 	...
 	$txd_user_code = isset($_POST['txd_user_code']) ? trim($_POST['txd_user_code']) : '';
	...
	/*ccx 2014-12-15 判断输入的六位短信验证码是否正确 开始*/
        if(empty($_POST['txd_user_code']) || strlen($txd_user_code) != 6 )
        {
            show_message($_LANG['passport_js']['txd_user_code_empty']);
        }
        $sql_count_ccx =  "SELECT id FROM ".$ecs->table('txd_user_code').
                          " WHERE account ='".$username."' AND code = " .$txd_user_code ." AND status = 0 AND type = 1 ";
        $record_count_ccx = $db->getOne($sql_count_ccx);
        if($record_count_ccx == '')
        {
            show_message("对不起，您输入的6位短信验证码不正确,请检查确认再次输入", $_LANG['sign_up'], 'user.php?act=register', 'error');
        }  
	    ...
	    /* ccx 2014-12-11 注册送红包功能 开始 start */
            $sql_hongbao = 'SELECT type_id FROM ' . $ecs->table("bonus_type").' WHERE send_type=4 ';  
            $reg_bonus = $db->getAll($sql_hongbao); 
            if($reg_bonus)
            {
                foreach ($reg_bonus AS $val)  
                  ...
                  $reg_bonus_message='  您注册已赠送红包';
            }
            /* ccx 2014-12-11 注册送红包功能 结束 end  */
            
            /*ccx 2014-12-16 手机注册成功之后，在user表的phone_validate字段标识变为1,表示通过手机注册验证的用户 开始*/
            $sql = 'UPDATE ' . $ecs->table('users') . 
                   " SET phone_validate = 1 " . 
                   " WHERE `user_id`='" . $_SESSION['user_id'] . "'";
            $db->query($sql);
            /*ccx 2014-12-16 手机注册成功之后，在user表的phone_validate字段标识变为1,表示通过手机注册验证的用户 结束*/
            /*ccx 2014-12-16 修改短信验证码的使用状态 已经使用：status 变为1  开始*/
            $sql = 'UPDATE ' . $ecs->table('txd_user_code') . 
                   " SET status = 1 " . 
                   " WHERE account ='" . $username . "' AND id=".$record_count_ccx;
            $db->query($sql);
            /*ccx 2014-12-16 修改短信验证码的使用状态 已经使用：status 变为1  结束*/
...
elseif ($action == 'profile')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $ecs->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id <>100 AND id<>101 ORDER BY dis_order, id';
    $extend_info_list = $db->getAll($sql);
...
elseif ($action == 'order_detail')
{
	...
	/* add 2014-10-08*/
	$order['confirm_time']  = str_replace('确认于','',$order['confirm_time']);
	$order['pay_time']      = str_replace('付款于','',$order['pay_time']);
	$order['shipping_time'] = str_replace('发货于','',$order['shipping_time']);
...
elseif ($action == 'collection_list')
{
    ...
    $smarty->display('collection_list.dwt');
}
...
{
	/*关闭手机端充值账户余额 2014-06-25 by hg*/
	show_message('请用电脑登陆本站进行充值', $_LANG['back_to_cart'], 'user.php?act=account_log');
...
elseif ($action == 'account_detail')
{
	...
	if($_REQUEST['json'] == '1')
	{
		json_dateli_list($account_log);die;
	}
    $smarty->display('account_detail.dwt');
}
...
elseif ($action == 'account_log')
{
    ...
    $smarty->assign('account_log',    $account_log);
    $smarty->assign('pager',          $pager);
    $smarty->display('account_log.dwt');
}
...
/* ccx 2014-12-21 开始*/
/* 新增 ccx 2014-12-21 发送短信  开始 */
/* 验证用户注册用户名是否可以注册 并且获取手机号码*/
...
elseif($action == 'send_mobil_message')
{
    ...
    $sql_number = "SELECT count(*) FROM ".$ecs->table('txd_user_code').
                  " WHERE account='".$username."'"; 
    $user_send_number = $db->getOne($sql_number); 
    if($user_send_number < 5 )  // 短信发送超过6次就不允许再进行发送了
    {
    
        /*ccx 2014-12-15 发送短信*/
        $MobileNotice = new MobileNotice() ; 
        $send_message = $MobileNotice->send($username, $code);
        //print_r($send_message);
        if($send_message == '000')
        {
            //提交成功
            //echo "短信发送成功";
            $sql = "INSERT INTO ".$ecs->table('txd_user_code').
                      " ( account, code, log_time, type, status )".
                   "VALUES('".$username."', $code, ".gmtime().", 1, 0 )";  
            $db->query($sql);
            echo "true";
        }
        elseif($send_message =='001')
        {
            //无效的商户编号
            echo "001";
        }
        elseif($send_message =='002')
        {
            //签名验证失败
            echo "002";
        }
        elseif ($send_message =='003') 
        {
            //系统错误
            echo "003";
        }
        elseif ($send_message =='004') 
        {
            //系统错误
            echo "004";
        }
        elseif ($send_message =='005') 
        {
            //手机号码错误
            echo "005";
        }
        elseif ($send_message =='006') 
        {
            //错误的数据格式，检查您的数据编码
            echo "006";
        }
        /*ccx 2014-12-15 发送短信结束*/
...
/* ccx 2014-12-14 发送短信  结束 */

function json_order_list($orders)
{
/* 异步返回html内容 */
	if(!$orders) echo '';
	$html = '';
	...
	$html .= '<li>
		  <a href="user.php?act=order_detail&order_id='.$value['order_id'].'" class="item-link item-content">
			<div class="item-media">
			  <img src=/'.$value['goods_thumb'].' width="60" height="60" />
			</div>
			<div class="item-inner">
			  <div class="item-title">订单号：'.$value['order_sn'].'</div>
			  <div class="item-date">下单日期：'.$value['order_time'].'</div>
			  <div class="item-other">
				总价：<strong class="orange">'.$value['total_fee'].'</strong> <br />
				状态：<strong class="orange">'.$value['order_status'].'</strong>
			  </div>
			</div>
		  </a>
		</li>';
...
/* 红包列表 */
function json_bonus_list($bonus)
{
/* 异步返回html内容 */
	if(!$bonus) echo '';
	$html = '';
	foreach($bonus as $key=>$value){
       $html .= '<div class="item">
				红包序号：'.$value['bonus_sn'].'<br />
				红包名称：'.$value['type_name'].'<br />
				红包金额：'.$value['type_money'].'<br />
				最小订单金额：'.$value['min_goods_amount'].'<br />
				<strong class="orange">截止日期：'.$value['use_enddate'].'</strong><br />
				<strong class="orange">状态：'.$value['status'].'</strong>
            </div>';
	}
	echo $html;
}
...
/* 红包列表 */
function json_dateli_list($dateli)
{
/* 异步返回html内容 */
	if(!$dateli) echo '';
	$html = '';
	foreach($dateli as $key=>$value){
		$html .= '<li class="item-content">
				<div class="item-inner">
				操作时间：'.$value['change_time'].'<br />
				类       型：'.$value['type'].'<br />
				金       额：'.$value['amount'].'<br />
				备注：'.$value['short_change_desc'].'<br />
				</div>
			  </li>';
	}
	echo $html;
