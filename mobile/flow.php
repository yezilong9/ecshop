 ...
 * Modify:
 * m1 SouthBear 2014-12-08 增加货到付款和在线支付
 */
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'], '', 'flow.php', 'warning');
    }
    ...
    /*如果某种配送方式同时设置了全国的运费和该省份的运费，过滤掉全国的运费，只使用该省份的运费 add by 2014-09-15 20:30*/
    $s_id = array();
    ...
    if (isset($_SESSION['APP'])) {
        ...
        for($i = 0;$i < count($show_bank);$i++)
        {
            $payment_list[] = array
            (
                'pay_id' => $show_bank[$i]['pay_id'],//银行支付代码
                'pay_code' => $show_bank[$i]['bank_code'],
                'pay_name' => $show_bank[$i]['bank_name'],
                'pay_fee' => 0,
                'pay_config' => '',
                'pay_desc' => '',
                'is_wap' => true,
                'bank_img' => $show_bank[$i]['bank_img'],
	...
	$payment_list = array_merge($payment_list,$arr_support_cod);
	...
	//m1 SouthBear 2014-12-09 注释掉只限制APP使用天下支付
	//if(isset($_SESSION['APP'])) {
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        //应对圣诞节活动 做出针对产品限制红包
        /* 2015-03-09 ccx 进行了注释 新增下面的针对某个商品才能进行使用该红包的功能
        $arr_christmas_goods_id_10 = array(10755,10756,10757,10758,10759,10760,10761,10762,10763,10764,10765,10766,10767,10768,10769,10770,10771,10774,10775,10778,10779,10780,10781,10782,10783,10784,
            10785,10786,10787,10788,10789,10790,10791,10792,10793,10794,10795,10796,10797,10798,10799,10800,10801,10802,10803,10804,10805,10806,10807
        );
        $arr_christmas_goods_id_20_30_50 = array(10779,10780,10781,10782,10783,10784,10785,9423);
        ...
        */
        ...
        if (!empty($user_bonus))
        {
            ...
            foreach ($user_bonus AS $key => $val)
            {
                ...
                foreach($cart_goods as $k2=>$v2)
                {
                    //if($v2['goods_id'] == 10791 && $val['type_money'] == 30)  //商品的id，以及红包的金额，到时候需要填写服务器上面的数据
                    if($v2['goods_id'] == 10791 && $val['type_id'] == 20)  //商品的id，以及红包的id，到时候需要填写服务器上面的数据
                    {
                        $bonus = true;
                        break;
                    }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    {//WAP支付
        $_POST['pay_id'] = $_POST['pay_id_'.$_POST['payment']];
    }
    $order = array(
        'shipping_id'     => intval($_POST['shipping']),
        'pay_num'          => $_POST['payment'],//wap支付代码
        'is_wap'          => $_POST['is_wap_'.$_POST['payment']],//wap支付代码
   ...
   // if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    if ($order['pay_id'] == '1' && $order['order_amount'] > 0)//update 2014-06-24 by hg
    {
    ...
    if($order['pay_id'] == '1' && date('Ym') == '201412')
    {
    ...
    if ($order['order_amount'] > 0)
    {
		 ...
		 */
		if ($order['pay_id'] == 2) {
			$payment['pay_code'] = 'cod';
		} else {
        	$payment = payment_info($order['pay_id']);
		}
        
		$payment['pay_code'] = strtolower($payment['pay_code']);
/*
echo '<pre>';
print_r($order);
print_r($_POST);
...
exit;*/
		 ...
		 */
		if ($order['pay_id'] != 2) {
			$pay_obj->config($order['pay_id'],$order);
			if($order['is_wap'])
			{//wap支付
			    $pay_obj->get_wap_token();
			    $smarty->assign('payment_method','wap');
			}
			else
			{
                $pay_obj->get_token();
        	...
        	$smarty->assign('pay_online', $order['order_id']);
    	}
    }
...
/* 前往天下支付 */
elseif($_REQUEST['step'] == 'txd_pay')
{
    /* 天下店wap支付 2014/12/16 14:32:20 weichen start*/
    $payment_method = isset($_GET['payment_method']) ? $_GET['payment_method']:'';
    $order_id = isset($_GET['order_id']) ? $_GET['order_id']:'';
    $order_id = 0 + $order_id ;
    if(($payment_method == 'wap' && $order_id))
    {
        include_once('includes/modules/payment/txd.php');
        $pay_obj    = new txd;
        $pay_obj->txd_wap_submit($order_id);
        show_message('支付中,请稍后');
    }
    /* 天下店wap支付 2014/12/16 14:32:20 weichen end*/
	if($order_id)
	{
		$token_id = $db->getRow("SELECT txd_pay_token_id FROM ".$ecs->table('order_info')." WHERE order_id = $order_id");
		header("Location:txd://token_id?$token_id");
	}
	show_message('支付错误');
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
    ...
    if (isset($_POST['rec_id']) &&isset($_POST['goods_number']))
    {
		...
		$cart_goods_id = $GLOBALS['db']->getRow("select goods_id,goods_number from ".$GLOBALS['ecs']->table('cart')." where rec_id = $key");
		$goods_start_num = $GLOBALS['db']->getOne("select start_num from ".$GLOBALS['ecs']->table('goods')." where goods_id = $cart_goods_id[goods_id]");
		
		if($val < $goods_start_num)
		{
			$result['error']   = 7;
            $result['message'] = '更新失败，购买数量低于起购数量';
            $result['goods_number'] = $cart_goods_id['goods_number'];
            $result['rec_id'] = $key;
			die($json->encode($result));
		}
...
//dump($cart_goods['goods_list']);

/* 载入不同的页面 */
if($_REQUEST['step'] == 'cart')
{
	$smarty->display('cart.dwt');exit;
}
if($_REQUEST['step'] == 'checkout')
{
	$smarty->display('order_desc.dwt');exit;
}
if($_REQUEST['step'] == 'done')
{
	$smarty->display('done.dwt');exit;
}
if($_REQUEST['step'] == 'login')
{
	$smarty->display('flow_login.dwt');exit;
}
if($_REQUEST['step'] == 'consignee')
{
	$smarty->display('consignee.dwt');exit;
}
/* end */
