...
/* 异步返回html内容 */
function json_goods_list($goodslist)
{
	if(!$goodslist) echo '';
	$html = '';
	foreach($goodslist as $key=>$value){
	  $html .= '<div class="item">
		<a href='.$value['url'].'>
		  <div class="pic"><img src='.$value['goods_img'].' width="145" /></div>
		  <p class="item-info">
			<span class="item-name">'.$value['name'].'</span>
			<strong class="item-price">'.$value['shop_price'].'</strong>
		  </p>
		</a>
	  </div>';
	}
