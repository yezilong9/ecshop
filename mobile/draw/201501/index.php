  ...
  <div class="popup popup-login">
    ...
    <div class="bd popup__bd">
        ...
        <div class="form-apply">
          <button title="登录" class="txtoverflow btn-submit" id="sub_button" >登 录</button>
        </div>
        <div class="form-apply">
        <a href="../../user.php?act=register&activity=login_draw_activity&url_request=<?=$url_request?>" target="_blank" style="color:#00F; text-decoration:underline" >没有账号，马上注册</a>
        </div>
    </div>
    <!--模块主体(end)-->
  </div>
...
$('#sub_button').click(function(e){
 ...
 var url = "../../login_action.php?act=login&";
 $.ajax({  
    type: "post",  
    url: url,  
    dataType: "json",  
    ...
    {  
        ...
        {
           //alert("登陆失败,该用户名未被注册,可前往注册进行抽奖");
           if(confirm('该用户不存在,请去注册页面注册账号再进行抽奖,谢谢合作')){window.location.href='../../user.php?act=register&activity=login_draw_activity&url_request=<?=$url_request?>'}; 
