...
class integrate
{
    ...
    {
        if ($this->check_user($username, $password) > 0 && $this->agency_user($username, $password))
        {
            if ($this->need_sync)
            {
	...
	function agency_user($username, $password)
	{
		...
		if(!empty($user_id))
		{
			...
			if($agency_user_id)
			{
				...
				$top_rank = $this->db->getOne("select top_rank from " . $this->table("users") . "where user_id = $user_id");
				if($top_rank != $agency_user_id){
					return false;
				}else{
					return true;
				}
