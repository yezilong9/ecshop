...
function get_categories_tree($cat_id = 0)
{
	...
	/* 根据代理商过滤分类 add by hg 2014-10-27*/
	$admin_agency_id = admin_agency_id()?admin_agency_id():agency_id();
	if($admin_agency_id) 
		$where = " AND (FIND_IN_SET('$admin_agency_id',agency_cat)  OR host_cat = 1) ";
    else
 ...
 *
 * @access  public
 * @param   string      $type       推荐类型，可以是 special,best, new, hot
 * @return  array
 */
       ...
       // if ($data === false)
        //{
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               ' WHERE '.$agency_where.' g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['shop_price']   = str_replace('元','',price_format($row['shop_price']));
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $row['shop_price_formated'] = str_replace('元','',price_format(get_final_price($goods_id)));
