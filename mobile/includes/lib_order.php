...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            " WHERE enabled = 1 AND pay_code = 'TXD' ";
    if (!$support_cod)
    {
...
/**
 * 取得wap可用的支付方式列表
 * @param   bool    $support_cod        支付商代码
 * @return  array   配送方式数组
 */
function wap_payment_list($payment = array())
{
    $sql = "SELECT sb.id,sb.bank_name,sb.bank_code,sb.bank_img,sb.Platform_pay,pb.pay_id,pb.bank_pay_num 
            FROM ".$GLOBALS['ecs']->table('show_bank'). " sb,".$GLOBALS['ecs']->table('pay_bank')." pb 
          WHERE sb.bank_code = pb.bank_code
            AND sb.state = 1  
            AND sb.Platform_pay = 1
            AND pb.enabled = 1
            AND pb.is_delete = 0";
    if($payment['payment'])
    {//支付商户
        $sql .= " AND sb.payment = '".$payment['payment']."'";
    }
    
    if($payment['pay_id'])
    {//支付商户id
        $sql .= " AND pb.pay_id = '".$payment['pay_id']."'";
    }
    
    if($payment['bank_code'])
    {//支付商户id
        $sql .= " AND sb.bank_code = '".$payment['bank_code']."'";
    }
