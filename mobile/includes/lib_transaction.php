...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$goods_thumb = '';
		$goods_thumb = $GLOBALS['db']->getOne("SELECT g.goods_thumb FROM ".$GLOBALS['ecs']->table('order_goods').
		" AS o LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id = o.goods_id WHERE o.order_id = $row[order_id]");
	
        $arr[] = array('order_id'       => $row['order_id'],
                       'order_sn'       => $row['order_sn'],
                       'order_time'     => local_date($GLOBALS['_CFG']['date_format'], $row['add_time']),
