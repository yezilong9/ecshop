...
function get_flash_xml()
{
    ...
    {
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\sagency="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/flash_data.xml'), $t, PREG_SET_ORDER))
        {
        ...
        if (!empty($t))
        {
            foreach ($t as $key => $val)
            {
					...
					$flashdb[] = array('src'=>$val[1],'url'=>$val[2],'text'=>$val[3],'sort'=>$val[4],'agency'=>$val[5]);
...
/* 根据不同代理商过滤首页广告 add by hg for date 2014-08-28 */
...
/*******************************************************************/
	...
	function vdump($arr)
	{
		echo '<pre>';
		$Array = func_get_args();
		array_walk($Array, create_function('&$item, $key', 'var_dump($item);'));
		echo '</pre>';
		exit();
	}
#end
