...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['market_price']  = str_replace('元','',price_format($row['market_price']));
        $goods_list[$row['goods_id']]['shop_price']    = str_replace('元','',price_format($row['shop_price']));
