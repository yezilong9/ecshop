...
if ($_REQUEST['act']== 'list')
{
	...
	$post_agency_id = isset($_POST['admin_agency_id'])?$_POST['admin_agency_id']:'';
	$smarty->assign('admin_agency_id',$post_agency_id);
    $xml = get_flash_xml();
	if(!$post_agency_id ){
		$where = agency_where();
		preg_match('/[0-9]{1,}/',$where,$agency_id);
		$agency_id = current($agency_id);
	}
	elseif(if_agency())
	{
		$agency_id = $post_agency_id;
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
		...
		elseif(if_agency())
		{
			$agency_user_id = $post_id;
		}
        // 插入新数据
        array_unshift($flashdb, array('src'=>$src, 'url'=>$_POST['img_url'], 'text'=>$_POST['img_text'] ,'sort'=>$_POST['img_sort'],'agency'=>$agency_user_id));
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $flashdb[$id] = array('src'=>$src,'url'=>$_POST['img_url'],'text'=>$_POST['img_text'],'sort'=>$_POST['img_sort'],'agency' =>$agency_user_id);
...
function put_flash_xml($flashdb)
{
    if (!empty($flashdb))
    {
        ...
        foreach ($flashdb as $key => $val)
        {
            $xml .= '<item item_url="' . $val['src'] . '" link="' . $val['url'] . '" text="' . $val['text'] . '" sort="' . $val['sort'] . '" agency="'.$val['agency'].'"/>';
        }
        $xml .= '</bcaster>';
...
function set_flash_data($tplname, $msg)
{
    ...
    if (empty($flashdata))
    {
        ...
        $flashdata[] = array(
                                ...
                                'text' => '',
                                'url' =>'http://help..com/index.php?doc-view-108.htm'
                            );
    }
    switch($tplname)
    {
...
function set_flash_uproll($tplname, $flashdata)
{
    ...
    {
        $xmldata .= '<myItem pic="' . $data['src'] . '" url="' . $data['url'] . '" agency= "' . $data['agency'] . '" />';
...
function set_flash_focus($tplname, $flashdata)
{
    ...
    $count = 1;
    $join = '';
	/*重写写入js文件 add by hg time 2014-05-29*/
 	$arr = array();
	
	foreach($flashdata as $key=>$value){
		$num = count($arr[$value['agency']])/3+1;
		$arr[$value['agency']]['imgUr'.$num.''] = $value['src'];
		$arr[$value['agency']]['imgtext'.$num.''] = $value['text'];
		$arr[$value['agency']]['imgLink'.$num.''] = $value['url'];
	}
	$jsdata = '';
	$var = '';
	foreach($arr as $arr_k=>$arr_v){
		$count = 1;
		$join = '';
		$jsdata .= 'arr['.$arr_k.'] = new Array();'."\n";
		$jsdata2[$arr_k]['url'];
		$jsdata2[$arr_k]['txt'];
		$jsdata2[$arr_k]['link'];
		foreach($arr_v as $data=>$data_v){
		
			$jsdata .= 'arr['.$arr_k.']["'.$data.'"] = "'.$data_v.'";'."\n";
			/* if ($count != 1)
			{
				$join = '+"|"+';
			}
			$jsdata2[$arr_k]['url']  .= $join . 'imgUrl' . $count;
			$jsdata2[$arr_k]['txt']  .= $join . 'imgtext' . $count;
			$jsdata2[$arr_k]['link'] .= $join . 'imgLink' . $count;
			 ++$count; */
		}
