...
if ($_REQUEST['act'] == 'list')
{
	...
	if(isset($article_list['arr'][0]['admin_agency_id'])){
		$url = and_agency_url($article_list['arr'][0]['admin_agency_id']);
		if($url) $url = 'http://'.$url;
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,admin_agency_id,allow,show_every) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]',".admin_agency_id().",$_POST[allow],$_POST[show_every])";
    $db->query($sql);
...
if ($_REQUEST['act'] =='update')
{
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]',allow=$_POST[allow],show_every=$_POST[show_every]", $_POST['id']))
    {
...
elseif($_REQUEST['act'] == 'show_every')
{//是否全站通告
    check_authz_json('article_manage');
    ...
    $exc->edit("show_every = '$val'", $id);
    clear_cache_files();
    make_json_result($val);
}
elseif($_REQUEST['act'] == 'allow')
{//是否全站通告
    check_authz_json('article_manage');
