...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $_POST['txmb_pwd'] = isset($_POST['txmb_pwd']) ? trim($_POST['txmb_pwd']) : '';
    ...
    if ($row)
    {
		/* 获取密保密码并验证 date 2014-07-04 begin*/
		$state = check_txmb($_POST['username'],$_POST['txmb_pwd']);
		if(!$state) sys_msg('验证密保失败', 1);
		/*end*/
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email,txmb_id, password, agency_id, role_id, real_name FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    $txmb_id = !empty($_REQUEST['txmb_id'])     ? trim($_REQUEST['txmb_id'])     : '';
    ...
    if (!empty($_POST['new_password']))
    {
        ...
        if (($old_password <> $old_ec_password) && ($_SESSION['action_list'] != 'all'))
        {
...
elseif ($_REQUEST['act'] == 'allot')
{
	...
	//dump($db->getAll("select * from ".$ecs->table('admin_action')));
    /* 赋值 */
    $smarty->assign('lang',        $_LANG);
...
/*验证密保*/
function check_txmb($username,$txmb_pwd){
	$txmb_id = $GLOBALS['db']->getOne("select txmb_id from ".$GLOBALS['ecs']->table('admin_user')." where user_name='$username'");
	if((int) ($txmb_id) < 1) return true;
	return preg_match('/^[0-9]{6}$/',$txmb_pwd)?txmb::check($txmb_id,$txmb_pwd):false;
}
	
/* 获取管理员列表 */
function get_admin_userlist()
{
/*add by hg for date 2014-04-18 筛选管理员与代理商身份*/
	$admin_agency_id = isset($_POST['admin_agency_id'])?$_POST['admin_agency_id']:0;
	if(!empty($admin_agency_id))
	{
		if($admin_agency_id == 1)
		{
			$where = 'where agency_user_id is null';
		}
		elseif($admin_agency_id == 2)
		{
			$where = 'where agency_user_id > 0';
		}
	}
	$GLOBALS['smarty']->assign('admin_agency_id',$admin_agency_id);
/*end*/
    $list = array();
    $sql  = "SELECT r.role_name,agency_user_id, user_id, user_name, email, add_time, last_login,IFNULL(a.real_name,'/') as real_name ".
            'FROM ' .$GLOBALS['ecs']->table('admin_user')." as a LEFT JOIN " . $GLOBALS['ecs']->table('role') . " AS r on r.role_id = a.role_id $where ORDER BY user_id DESC";
