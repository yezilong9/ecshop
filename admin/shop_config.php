...
if ($_REQUEST['act'] == 'list_edit')
{
    ...
    /* 修改短信设置的一个项：将 商家的手机号码 修改为 是否启用手机短信服务 */
	$sql_sms_code = "SELECT code FROM " .$ecs->table('shop_config')." WHERE id=801 AND parent_id=8";
	$sms_code = $db->getOne($sql_sms_code);
	if($sms_code != 'is_start_sms'){
		$sql_update_sms_code = "UPDATE " .$ecs->table('shop_config')." SET code='is_start_sms',type='select',store_range='1,0',value=0 WHERE id=801 AND parent_id=8";
		$db->query($sql_update_sms_code);
	}
	
	
	/* 检查权限 */
    admin_priv('shop_config');
	/*add by hg for date 2014-05-04 重写商品配置信息*/
	$admin_agency_id = (!empty($_REQUEST['admin_agency_id'])) ? $_REQUEST['admin_agency_id'] :'';//代理商id
	
	//exit();
	if(!if_agency() || !empty($admin_agency_id))
	{
		agency_shop_config($admin_agency_id);
	}
	
	//dump($_CFG);
	//判断是否是代理商
	$action_list = if_agency()?'all':'';//if_agency()
	...
	if(if_agency() && empty($admin_agency_id))
	{
		...
		$smarty->assign('group_list',   get_settings(null, array('2','3','4','5','6','7','8','9')));
	}
	...
	//dump(get_settings(null, array('5')));
...
/*add by hg for date 2014-05-04 查找id最大值与最小值 begin*/
	$admin_agency_id = (!empty($_REQUEST['admin_agency_id'])) ? $_REQUEST['admin_agency_id'] :'';//代理商id
	if(!if_agency() || !empty($admin_agency_id))
	{
		$shopRes = $db->getAll("select max(id) as max_id ,min(id) as min_id from ".$ecs->table('shop_config')." where parent_id = 1");
		
		$agency_where = agency_where($admin_agency_id);
		$agencyRes = explode(' ',$agency_where);
		$agency_id = $agencyRes[4];
		$agencyShopRes = $db->getRow("select max(id) as max_id ,min(id) as min_id from ".$ecs->table('agency_shop_config')." where parent_id = 1 $agency_where");
		//dump($agencyShopRes);
	}
/*end*/
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
		...
		{ //代理商设置商店时
			if($key<$shopRes['min_id'] && $key>$shopRes['max_id'])
			{
				if($arr[$key] != $val)
				{
					$sql = "UPDATE " . $ecs->table('shop_config') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
			...
			{
				$sql = "UPDATE " . $ecs->table('agency_shop_config') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
				$db->query($sql);
			}
		}
    }
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                if ($code == 'shop_logo')
                {
					...
					/*add by hg for date 2014-05-04 logo重命名*/
					if(if_agency() && empty($admin_agency_id))
					{
					...
					{
						$lo = explode('.',$info['logo']);
						$info['logo'] = $lo[0].$agency_id.'.'.$lo[1];
						$file_name = str_replace('{$template}', $_CFG['template'], $file_var_list[$code]['store_dir']) . $info['logo'];
					} 
                ...
                if (move_upload_file($file['tmp_name'], $file_name))
                {
					...
					{//写入代理商表
						$sql = "UPDATE " . $ecs->table('agency_shop_config') . " SET value = '$file_name' WHERE code = '$code' $agency_where";
						$db->query($sql);
						//dump($sql);
					}
    ...
    $spt = '<script type="text/javascript" src="http://api..com/record.php?';
    ...
    {
        $links[] = array('text' => $_LANG['back_shop_config'], 'href' => 'shop_config.php?act=list_edit&admin_agency_id='.$agency_id.'');
...
function get_settings($groups=null, $excludes=null)
{
	...
	if(if_agency() && empty($admin_agency_id))
	{
		$sql = "SELECT * FROM " . $ecs->table('shop_config') .
            " WHERE $hide type<>'hidden' $config_groups $excludes_groups ORDER BY parent_id, sort_order, id";
	}
	else
	{ // add by hg for date 2014-05-04 代理商
		$sql = "SELECT * FROM " . $ecs->table('shop_config') .
            " WHERE $hide type<>'hidden' AND parent_id <> 1 $config_groups $excludes_groups ORDER BY parent_id, sort_order, id";
	}
	...
	{
		$agency_where = agency_where($admin_agency_id);
		$agency_list = $db->getAll("SELECT * FROM " . $ecs->table('agency_shop_config') .
            " WHERE $hide type<>'hidden' $agency_where $config_groups $excludes_groups ORDER BY parent_id, sort_order, id");
