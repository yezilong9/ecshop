...
//        break;
    ...
    case 'display_send_ui' :
        ...
        if ($sms->has_registered())
        {
            ...
            /*ccx 2014-12-29  start 新增按照手机注册用户发送短信*/
            $send_rank['9_99'] = "手机验证用户";
            /*ccx 2014-12-29  end*/
            assign_query_info();
    ...
    case 'send_sms' :
        ...
        if ($send_rank != 0)
        {
            ...
            /*ccx 2014-12-29 start 新增按照手机注册用户发送短信*/
            elseif($rank_array['0'] == 9 && $rank_array['1'] == 99 )
            {
                $link[] = array('text'  =>  $_LANG['back'] . $_LANG['03_sms_send'],
                        'href'  =>  'sms.php?act=display_send_ui');
                $sql = 'SELECT user_name FROM ' . $ecs->table('users') . "WHERE phone_validate = 1 ";
                $row = $db->query($sql);
                while ($rank_rs = $db->fetch_array($row))
                {
                    if($rank_rs) //判断是否存在手机号码
                    {   
                        if(!preg_match("/1[3458]{1}\d{9}$/",$rank_rs['user_name']))  //判断手机号码是否正确
                        {
                            sys_msg('该手机号码:'.$rank_rs['user_name'].'格式不正确,请认真检查手机号码是否正确', 0, $link,false);
                        }
                    }
                    $value[] = $rank_rs['user_name'];
                }
            }
            /*ccx 2014-12-29 end */
            else
        ...
        $send_date = isset($_POST['send_date']) ? $_POST['send_date'] : date('Y-m-d H:i:s');   
        
		$phone_num = array();
		$phone_num = explode(',',$phone);

		$send_content = $msg;//.'发送于'.$send_date
		$c_num = count($phone_num);
		$link[] = array('text'  =>  $_LANG['back'] . $_LANG['03_sms_send'],
                        'href'  =>  'sms.php?act=display_send_ui');
						
		for($i = 0;$i < $c_num; $i++){
			if(!empty($phone_num[$i])){
				$obj_sms = new class_sms($phone_num[$i],$send_content);
                $result = $obj_sms->send_sms();//发送短信
				$result_arr = json_decode($result,true);
                
				if ($result_arr['code'] != 0){
				    sys_msg($phone_num[$i].'发送不成功，错误码：'.$result_arr['code'].'，错误信息：'.$result_arr['message'], 0, $link,false);
					
				}
			}
		}
		sys_msg($_LANG['send_ok']."共：".($c_num - 1), 0, $link,false);
