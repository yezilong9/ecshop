...
function get_user_rank_list()
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_rank') .
           " WHERE 1 $agency_where ORDER BY min_points";
...
/**
 * 保存某商品的会员价格和该商品对应会员等级的起购数量
 * @param   int     $goods_id   商品编号
 * @param   array   $rank_list  等级列表
 * @param   array   $price_list 价格列表
 * @return  void
 */
function handle_member_price($goods_id, $rank_list, $price_list,$start_num_list)
{	//print_r($rank_list);
    /* 循环处理每个会员等级 */
    ...
    { 
		...
		/* 会员等级对应的起购数量 */
		$rank_goods_start_num = $start_num_list[$key];
        ...
        {	
			/* 更新会员等级对应的起购数量和价格*/
			$condition = " SET user_price = '$price' , rank_goods_start_num = '$rank_goods_start_num' ";
        	if($price<=0 && $rank_goods_start_num<=0)
        		$condition = " SET user_price = NULL , rank_goods_start_num = NULL ";
        	if($price<=0 && $rank_goods_start_num>0)
        		$condition = " SET user_price = NULL , rank_goods_start_num = '$rank_goods_start_num' ";
        	if($price>0 && $rank_goods_start_num<=0)
        		$condition = " SET user_price = '$price' , rank_goods_start_num = NULL";
        	$sql = "UPDATE " . $GLOBALS['ecs']->table('member_price') 
        	.$condition." WHERE goods_id = '$goods_id' " ." AND user_rank = '$rank' LIMIT 1";
        ...
        { 
			/* 插入会员等级对应的起购数量和价格*/
        	$upd_val = " VALUES('$goods_id', '$rank', $price, '$rank_goods_start_num')";
        	if($price<=0 && $rank_goods_start_num<=0)
        		$upd_val = " VALUES('$goods_id', '$rank', NULL, NULL)";
        	if($price<=0 && $rank_goods_start_num>0)
        		$upd_val = " VALUES('$goods_id', '$rank', NULL, '$rank_goods_start_num')";
        	if($price>0 && $rank_goods_start_num<=0)
        		$upd_val = " VALUES('$goods_id', '$rank', $price, NULL)";
			
        	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('member_price') . "(goods_id, user_rank, user_price, rank_goods_start_num) " .$upd_val;
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
	...
	$filter['is_purchase'] = isset($_REQUEST['is_purchase']) ? ((empty($_REQUEST['is_purchase']) && $_REQUEST['is_purchase'] === 0) ? '' : trim($_REQUEST['is_purchase'])) : '';
		...
		if ($filter['is_purchase'] !== '')
        {
            $where .= " AND (is_purchase = '" . $filter['is_purchase'] . "')";
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods_supplier'). " AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT cat_id,start_num,host_goods_id,goods_id, goods_name, goods_type, goods_sn, shop_price, market_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,costing_price, integral,admin_agency_id, province_name,city_name,area_name, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote, is_purchase ".
                    " FROM " . $GLOBALS['ecs']->table('goods_supplier') . " AS g WHERE is_delete='$is_delete' $where" .
	...
	foreach($row as $row_k=>$row_v){
		/*ccx 2014-11-14 edt 读取相应的分类的名称*/	
		$row[$row_k]['cat_name'] =$GLOBALS['db']->getOne("SELECT cat_name FROM ".$GLOBALS['ecs']->table('category')." WHERE cat_id = $row_v[cat_id] ");			
		/*证明是代理商添加的商品*/
		if($row_v['host_goods_id']=='0' && $row_v['admin_agency_id']){
			$row[$row_k]['agency_goods'] = '1';
		}elseif(!$row_v['admin_agency_id']){//主站添加的商品
			//代理商起购数量
			$res = $GLOBALS['db']->getRow("SELECT user_price,rank_goods_start_num FROM ".
			$GLOBALS['ecs']->table('member_price')." WHERE goods_id = $row_v[goods_id] AND user_rank=4");
			if($res['rank_goods_start_num']){
				$row[$row_k]['rank_goods_start_num'] = $res['rank_goods_start_num'];
			}else{
				$rank_start_num = $GLOBALS['db']->getOne("SELECT rank_start_num FROM ".
				$GLOBALS['ecs']->table('user_rank')." WHERE rank_id = 4");
			...
			if($res['user_price'])
			{
				...
				$discount = $GLOBALS['db']->getOne("SELECT discount FROM ".
				$GLOBALS['ecs']->table('user_rank')." WHERE rank_id = 4");
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
	...
	$img_strrpos =  strrpos($source_img, '.');
    $img_ext = substr($source_img,$img_strrpos);
    $dir = 'images';
