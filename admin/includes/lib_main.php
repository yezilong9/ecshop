...
function get_where_sql($filter)
{
    ...
    /*获取代理商的admin_agency_id  ccx 2014-12-11 */
    $admin_agency_id_ccx  = isset($filter->admin_agency_id) ? trim($filter->admin_agency_id) : '';
    ...
    elseif(if_agency())
    {
        if(!empty($admin_agency_id_ccx))
        {
            $agency_where= " admin_agency_id = $admin_agency_id_ccx  AND ";
        }
        else 
        {
           $agency_where = " admin_agency_id = 0  AND "; 
        }
    }
    /*end*/

    $where  = isset($filter->is_delete) && $filter->is_delete == '1' ?
        ' WHERE '.$agency_where.' is_delete = 1 ' : ' WHERE '.$agency_where.' is_delete = 0 ';
...
**/
function hg_cat_exists($cat_name, $parent_cat,$admin_agency_id)
{
	$where = '';
	if(!$admin_agency_id)
		$where = ' AND host_cat = 1';
	else
		$where = " AND agency_cat like '%,$admin_agency_id,%'";
    $sql = "SELECT cat_id FROM " .$GLOBALS['ecs']->table('category').
    " WHERE parent_id = '$parent_cat' AND cat_name = '$cat_name' AND cat_id<>'$exclude' $where";
    return $GLOBALS['db']->getOne($sql);
}
