...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'download')
{
    ...
    /* 获取访问购买的比例数据 */
    $click_sold_info = click_sold_info($cat_id, $brand_id, $show_num,$admin_agency_id);
	...
	/*end*/
    $filename = 'visit_sold';
    $smarty->assign('action_link',  array('text' => $_LANG['download_visit_buy'], 'href' => 'visit_sold.php?act=download&show_num=' . $show_num . '&cat_id=' . $cat_id . '&brand_id=' . $brand_id . '&show_num=' . $show_num . '&admin_agency_id=' . $admin_agency_id));
 ...
 */
 function click_sold_info($cat_id, $brand_id, $show_num,$admin_agency_id)
 {
    ...
    $sql = "SELECT og.goods_id, g.goods_sn, g.goods_name, g.click_count,  COUNT(og.goods_id) AS sold_times ".
        " FROM ". $ecs->table('goods') ." AS g, ". $ecs->table('order_goods') ." AS og, " .$ecs->table('order_info') . " AS o " . $where.$sale_where .
