...
if ($_REQUEST['act'] == 'list')
{
    ...
    $list = group_buy_list();
	/* 重写团购活动订单url */
	if(isset($list['item'][0]['admin_agency_id'])){
		$url = and_agency_url($list['item'][0]['admin_agency_id']);
...
function group_buy_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods_activity') .
                " WHERE act_type = '" . GAT_GROUP_BUY . "' $where  $agency_where";
