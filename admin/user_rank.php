...
if ($_REQUEST['act'] == 'list')
{
	...
	$arr_res = agency_list();
    $smarty->assign('agency_list',   $arr_res);
	//代理商条件
    ...
    $ranks = array();
    $ranks = $db->getAll("SELECT * FROM " .$ecs->table('user_rank')." where 1 $agency_where");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查是否存在重名的会员等级 */
    if (!$exc->is_only('rank_name', trim($_POST['rank_name']),0,'admin_agency_id='.admin_agency_id()))
    {
    ...
    $sql = "INSERT INTO " .$ecs->table('user_rank') ."( ".
                "rank_name, min_points, max_points, discount, special_rank, show_price,admin_agency_id,rank_start_num".
            ") VALUES (".
                "'$_POST[rank_name]', '" .intval($_POST['min_points']). "', '" .intval($_POST['max_points']). "', ".
                "'$_POST[discount]', '$special_rank', '" .intval($_POST['show_price']). "',".admin_agency_id().",$_POST[rank_start_num])";
    $db->query($sql);
 ...
 *  修改等级会员相应的起购数量
 */
elseif ($_REQUEST['act'] == 'edit_rank_start_num')
{
    check_authz_json('user_rank');
    ...
    if ($val < 1)
    {
        $_LANG['js_languages']['rank_start_num_invalid'] = '你输入的数量不能小于1，且需要为整数！';
        make_json_error($_LANG['js_languages']['rank_start_num_invalid']);
    }

    if ($exc->edit("rank_start_num = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
