...
if ($_REQUEST['act'] == 'send')
{
    ...
    {
        /*add by ccx  for date 2014-12-11  代理商列表 begin*/
...
{
    @set_time_limit(0);
$assign = '1003292950,1003284420,1003275312,1003268375,1003251675,1003244701,1003235763,1003220453,1003217892,1003209166,1003197420,1003187361,1003172991,1003168904,1003153716,1003142871,1003130831,1003127415,1003116203,1003104066,1003093572,1003086310,1003071279,1003062629,1003054774,1003047571,1003031818,1003024266,1003019113,1003001950,1002999188,1002980734,1002979394,1002966988,1002950373,1002946315,1002934807,1002920916,1002913846,1002909159,1002896534,1002881677,1002873309,1002866301,1002855353,1002846810,1002838362,1002829146,1002816961,1002807743,1002796423,1002788760,1002777837,1002769472,1002757414,1002745509,1002735906,1002721692,1002710141,1002706640,1002690266,1002682218,1002672391,1002661240,1002656358,1002643927,1002635415,1002628297,1002612042,1002609953,1002598138,1002580347,1002577424,1002563183,1002553441,1002543003,1002533925,1002526313,1002512759,1002509930,1002499188,1002489142,1002477145,1002465246,1002455576,1002440023,1002430329,1002429480,1002410047,1002403648,1002395749,1002383454,1002374668,1002367295,1002350596,1002344994,1002337571,1002329209,1002318501,1002300961,1002298610,1002280213,1002270638,1002261768,1002258976,1002247351,1003334157,1002220151,1002215915,1002208615,1002190049,1002189204,1002174972,1002160579,1002156150,1002143221,1002133491,1002127605,1002114629,1003324799,1002094908,1002087893,1002078213,1002063425,1002055116,1002044913,1002038358,1002024561,1002017661,1002009229,1001990418,1001982232,1001977025,1001968011,1001956400,1001942160,1001936460,1001924641,1001919326,1001903979,1001898850,1001889377,1001870410,1001868392,1001850370,1001843339,1001834917,1001829602,1001813181,1001806036,1001796925,1001780292,1001774390,1001767840,1001759811,1001743666,1001738008,1001728283,1001715246,1001708603,1001692631,1001681434,1001670956,1001669637,1001651335,1001644722,1001633925,1001627506,1001612243,1001604681,1001594912';
    /* 获得此线下红包类型的ID */
    $tid  = !empty($_GET['tid']) ? intval($_GET['tid']) : 0;
    $type_name = $db->getOne("SELECT type_name FROM ".$ecs->table('bonus_type')." WHERE type_id = '$tid'");
    ...
    if (EC_CHARSET != 'gbk')
    {
        ...
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['type_money']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['type_name']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '使用日期') ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '订单号') ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '使用会员') ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '收货人姓名' )."\t";
        echo ecs_iconv('UTF8', 'GB2312', '收货人电话号码') ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '收货人所在区域') ."\t";
        echo ecs_iconv('UTF8', 'GB2312', '收货人详细地址') ."\t\n";
    }
    else
    {
        echo $_LANG['bonus_excel_file'] . "\t\n";
	...
	$filter['bonus_sn']  = isset($_REQUEST['bonus_sn']) ? trim($_REQUEST['bonus_sn']) : '';
	...
	$filter['consignee'] = isset($_REQUEST['consignee']) ? trim($_REQUEST['consignee']) : '';
	$filter['tel'] 	     = isset($_REQUEST['tel']) ? trim($_REQUEST['tel']) : '';
	$filter['address']   = isset($_REQUEST['address']) ? trim($_REQUEST['address']) : '';
	$filter['state']     = isset($_REQUEST['state']) ? $_REQUEST['state'] : '';
	$filter['assign']     = isset($_REQUEST['state']) ? $_REQUEST['assign'] : '';
    /* 时间参数 */
	...
	/* 搜索条件 SQL */
	if(!empty($filter['bonus_sn']))
		$where .= " AND ub.bonus_sn like '%$filter[bonus_sn]%'";
	...
	if(!empty($filter['consignee']))
		$where .= " AND o.consignee like '%$filter[consignee]%'";
	if(!empty($filter['tel']))
		$where .= " AND o.tel like '%$filter[tel]%'";
	if(!empty($filter['address']))
		$where .= " AND o.address like '%$filter[address]%'";
	if($filter['state'] == 'true'){
		$where .= " AND ub.used_time >= '".$filter['start_date']."' AND ub.used_time < '" . ($filter['end_date'] + 86400)."'";
	}
	if($filter['assign'] == 'true')
		$where .= " AND ub.bonus_sn in ($assign) ";
    $val = array();
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_id,o.order_sn,o.consignee,o.address,o.tel, bt.type_name,r.region_name as province,re.region_name as city,reg.region_name as district ".
          ...
          " LEFT JOIN " .$GLOBALS['ecs']->table('order_info'). " AS o ON o.order_id=ub.order_id ".
          " LEFT JOIN " .$GLOBALS['ecs']->table('region'). " AS r ON o.province=r.region_id ".
          " LEFT JOIN " .$GLOBALS['ecs']->table('region'). " AS re ON o.city=re.region_id ".
          " LEFT JOIN " .$GLOBALS['ecs']->table('region'). " AS reg ON o.district=reg.region_id $where ".
    ...
    while ($val = $db->fetchRow($res))
    {
        ...
        echo $code_table[$val['type_name']] . "\t";
        echo local_date('Y-m-d H:i:s', $val['used_time']) . "\t";
		if (EC_CHARSET != 'gbk')
		{
			$code_table[$val['order_sn']]  = ecs_iconv('UTF8', 'GB2312', $val['order_sn']);
			$code_table[$val['user_name']] = ecs_iconv('UTF8', 'GB2312', $val['user_name']);
			$code_table[$val['consignee']] = ecs_iconv('UTF8', 'GB2312', $val['consignee']);
			$code_table[$val['tel']]       = ecs_iconv('UTF8', 'GB2312', $val['tel']);
			
			$code_table[$val['province']]       = ecs_iconv('UTF8', 'GB2312', $val['province']);
			$code_table[$val['city']]       = ecs_iconv('UTF8', 'GB2312', $val['city']);
			$code_table[$val['district']]       = ecs_iconv('UTF8', 'GB2312', $val['district']);
			
			$code_table[$val['address']]   = ecs_iconv('UTF8', 'GB2312', $val['address']);
		}
		else
		{
			$code_table[$val['order_sn']]  = $val['order_sn'];
			$code_table[$val['user_name']] = $val['user_name'];
			$code_table[$val['consignee']] = $val['consignee'];
			$code_table[$val['tel']]  	   = $val['tel'];
			
			$code_table[$val['province']]  	   = $val['province'];
			$code_table[$val['city']]  	   = $val['city'];
			$code_table[$val['district']]  	   = $val['district'];
			
			$code_table[$val['address']]   = $val['address'];
		}
		echo $code_table[$val['order_sn']] . "\t";
		echo $code_table[$val['user_name']] . "\t";
		echo $code_table[$val['consignee']] . "\t";
		echo $code_table[$val['tel']] . "\t";
		echo $code_table[$val['province']].$code_table[$val['city']].$code_table[$val['district']]. "\t";
		echo $code_table[$val['address']] . "\t";
        echo "\t\n";
    }
}
/**
...
function get_type_list()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['send_start_date'] = local_date('Y-m-d',$row['send_start_date']);
		$row['send_end_date'] =  local_date('Y-m-d',$row['send_end_date']);
...
$assign = '1003292950,1003284420,1003275312,1003268375,1003251675,1003244701,1003235763,1003220453,1003217892,1003209166,1003197420,1003187361,1003172991,1003168904,1003153716,1003142871,1003130831,1003127415,1003116203,1003104066,1003093572,1003086310,1003071279,1003062629,1003054774,1003047571,1003031818,1003024266,1003019113,1003001950,1002999188,1002980734,1002979394,1002966988,1002950373,1002946315,1002934807,1002920916,1002913846,1002909159,1002896534,1002881677,1002873309,1002866301,1002855353,1002846810,1002838362,1002829146,1002816961,1002807743,1002796423,1002788760,1002777837,1002769472,1002757414,1002745509,1002735906,1002721692,1002710141,1002706640,1002690266,1002682218,1002672391,1002661240,1002656358,1002643927,1002635415,1002628297,1002612042,1002609953,1002598138,1002580347,1002577424,1002563183,1002553441,1002543003,1002533925,1002526313,1002512759,1002509930,1002499188,1002489142,1002477145,1002465246,1002455576,1002440023,1002430329,1002429480,1002410047,1002403648,1002395749,1002383454,1002374668,1002367295,1002350596,1002344994,1002337571,1002329209,1002318501,1002300961,1002298610,1002280213,1002270638,1002261768,1002258976,1002247351,1003334157,1002220151,1002215915,1002208615,1002190049,1002189204,1002174972,1002160579,1002156150,1002143221,1002133491,1002127605,1002114629,1003324799,1002094908,1002087893,1002078213,1002063425,1002055116,1002044913,1002038358,1002024561,1002017661,1002009229,1001990418,1001982232,1001977025,1001968011,1001956400,1001942160,1001936460,1001924641,1001919326,1001903979,1001898850,1001889377,1001870410,1001868392,1001850370,1001843339,1001834917,1001829602,1001813181,1001806036,1001796925,1001780292,1001774390,1001767840,1001759811,1001743666,1001738008,1001728283,1001715246,1001708603,1001692631,1001681434,1001670956,1001669637,1001651335,1001644722,1001633925,1001627506,1001612243,1001604681,1001594912';
    /* 查询条件 */
    $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'bonus_type_id' : 'ub.'.trim($_REQUEST['sort_by']);
    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    $filter['bonus_type'] = empty($_REQUEST['bonus_type']) ? 0 : intval($_REQUEST['bonus_type']);
	/* 搜索条件 */
	$filter['bonus_sn']  = isset($_REQUEST['bonus_sn']) ? trim($_REQUEST['bonus_sn']) : '';
    ...
    foreach ($row AS $key => $val)
    {
        $row[$key]['used_time'] = $val['used_time'] == 0 ?
            $GLOBALS['_LANG']['no_use'] : '<span style="color:red">'.local_date($GLOBALS['_CFG']['time_format'], $val['used_time']).'</span>';
        $row[$key]['emailed'] = $GLOBALS['_LANG']['mail_status'][$row[$key]['emailed']];
        $row[$key]['area'] = $row[$key]['province'].$row[$key]['city'].$row[$key]['district'];
    }
	/* 导出execl 条件*/
    $GLOBALS['smarty']->assign('action_link2',  array('text' => '导出excel',
    'href'=>"#download"));
