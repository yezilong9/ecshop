...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($_REQUEST['act'] == 'edit')
    {
        ...
        /*ccx 2015-02-05 编辑申请 判断是提现还是充值，提现就显示开户信息*/
        if($user_account['process_type'] == 1)
        {
            $tx_bank = $db->getRow("SELECT * FROM " .$ecs->table('tx_bank') . " WHERE user_account_id = '$id'");
            $smarty->assign('tx_bank',     $tx_bank);
        }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 2015-02-05 ccx start 针对代理商，只能给归属自己的会员进行操作 主站能够对所有的会员进行提现、充值*/
    $admin_agency_id = admin_agency_id();
    if($admin_agency_id)
    {
        $sql_agency = "SELECT user_id FROM " .$ecs->table('users'). 
                      " WHERE user_name = '$user_name' AND top_rank = $admin_agency_id ";
        $user_id_agency = $db->getOne($sql_agency);
        /* 此会员是否存在代理商中 */
        if ($user_id_agency == 0)
        {
            $link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$ecs->table('user_account').
               " VALUES ('', '$user_id', '$_SESSION[admin_name]', '$amount', '".gmtime()."', '".gmtime().
                         "', '$admin_note', '$user_note', '$process_type', '$payment', '$is_paid', '0', '$admin_agency_id')";
        $db->query($sql);
        $id = $db->insert_id();
        
        /* 2015-02-05 ccx start 提现操作，需要填写开户人，开户银行，银行账号 */
        if ($process_type == 1)
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('tx_bank') . 
                   " (account_name, bank, bank_number, user_id, user_account_id ) " .
                   " VALUES ('$account_name', '$bank', '$bank_number', '$user_id', '$id')";
            $db->query($sql);
        }
        /* 2015-02-05 ccx end */
    }
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    {
		/* 取得银行信息 add by hg for date 2014-08-29 */
		$bank_res = $db->getRow("SELECT account_name,bank,bank_number FROM ".$ecs->table('tx_bank').
		" WHERE user_account_id = $account[id]");
		$smarty->assign('bank_res',$bank_res);
...
function tongji($sql)
{
	...
	$list = $GLOBALS['db']->getAll($sql);
	$rechargeable    = '';//充值成功金额
	$rechargeable_no = '';//充值未确定金额
	$withdraw_price  = '';//提现金额
	$withdraw_price_no  = '';//提现未确定金额
	$and_amount = '';
    foreach ($list AS $key => $value)
    {
		if($value['process_type'] == '0' && $value['is_paid'] == '1')
		{
			$rechargeable += $value['amount'];
		}
		if($value['process_type'] == '0' && $value['is_paid'] == '0')
		{
			$rechargeable_no += $value['amount'];
		}
		if($value['process_type'] == '1' && $value['is_paid'] == '1')
		{
			$withdraw_price += substr($value['amount'],1);
		}
		if($value['process_type'] == '1' && $value['is_paid'] == '0')
		{
			$withdraw_price_no += substr($value['amount'],1);
		}
		$value['amount'] = preg_replace("/-/",'',$value['amount']);

		$and_amount += $value['amount'];
    }
	$GLOBALS['smarty']->assign('rechargeable',       price_format($rechargeable));
	$GLOBALS['smarty']->assign('rechargeable_no',       $rechargeable_no);
	$GLOBALS['smarty']->assign('withdraw_price',       $withdraw_price);
	$GLOBALS['smarty']->assign('withdraw_price_no',       $withdraw_price_no);
	$GLOBALS['smarty']->assign('and_amount',       price_format($and_amount));
