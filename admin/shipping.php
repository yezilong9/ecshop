...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_code,shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " .$ecs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' ORDER BY shipping_order";
        $row = $db->GetRow($sql);
    ...
    $smarty->assign('ur_here', $_LANG['03_shipping_list']."<b style='font-weight:normal;color:red;'> (请点击对应配送方式后面的 设置运费 进行地区运费模板的设置。)</b>");
