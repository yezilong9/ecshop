...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    /*add by hg for date 2014-04-24 代理商列表  begin*/
    $admin_agency_id = (!empty($_REQUEST['admin_agency_id'])) ? intval($_REQUEST['admin_agency_id']) : 0;
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (!empty($_COOKIE['ECSCP']['lastfilter']))
    {
        ...
        if (!empty($filter['composite_status']))
        {
			...
			if(!empty($where))
			{
				...
				$where = $whereArr[0].$whereArr[1].' o.'.$whereArr[2].$whereArr[3].$whereArr[4];
			}
			/*end*/
            //综合状态
	...
	//$order['pay_name'] = $db->getOne("select bank_name from ".$ecs->table('pay_bank')." where local_bank_code = '$order[pay_num]'");
    /* 取得订单商品总重量 */
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr,g.supply_sign_id,g.host_goods_id,p.product_id, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
    ...
    {
		//dump($order[order_id]);
		/*add by hg for date 2014-04-30取主站商品实际库存*/
		/*if(!empty($row['host_goods_id']) && empty($row['product_id']))
		{
			$row['storage'] = $db->getOne("select goods_number from ".$ecs->table('goods')." where goods_id=$row[host_goods_id]");
		}*/
        /* 虚拟商品支持 */
        if ($row['is_real'] == 0)
        {
		...
		/* 如果是天猫的产品增加天猫的产品链接 */
		if(strpos($row['supply_sign_id'],'tm_') !== false)
		{
			$row['tm_url'] = 'http://detail.tmall.com/item.htm?id='.substr($row['supply_sign_id'],strlen('tm_'));
		}
        elseif($row['host_goods_id'])
        {
            $supply_sign_id = $db->getOne('SELECT supply_sign_id FROM '.$ecs->table('goods').
            " where goods_id = $row[host_goods_id]");
            if(strpos($supply_sign_id,'tm_') !== false)
            {
                $row['tm_url'] = 'http://detail.tmall.com/item.htm?id='.substr($supply_sign_id,strlen('tm_'));
            }
        }
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    /*ccx 2014-12-10 主站的订单只能主站去进行发货，代理商的订单只能是代理商去进行发货确认 开始*/
    $agency_id = $GLOBALS['db']->getOne("SELECT admin_agency_id FROM " . $ecs->table('order_info') . " WHERE order_id = $order_id ") ;
    if($admin_agency_id == NULL)
    {
        $admin_agency_id =0;
    }
    if($agency_id != $admin_agency_id)
    {
        $GLOBALS['db']->rollback();
        $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
        sys_msg($_LANG['act_false']."--请检查订单的相关信息，注意主站订单只能主站进行发货，代理商订单只能代理商进行发货处理", 1, $links);
    }
    /*ccx 2014-12-10 主站的订单只能主站去进行发货，代理商的订单只能是代理商去进行发货确认 结束*/

    /** ccx   根据订单的相关信息，读取商品的库存并且进行比较，库存操作 开始 start **/
    
    $order_message = $GLOBALS['db']->getAll("select rec_id, order_id, goods_id, goods_number, goods_name  from " . $ecs->table('order_goods') ." where order_id = $order_id");
    foreach($order_message as $costing_key=>$costing_value)
    {
        $order_sn = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $ecs->table('order_info') . " WHERE order_id = '".$costing_value['order_id']."' ") ;
           
        $sql = "select sum(goods_number)  from " . $ecs->table('stock_control') ." where goods_id = '".$costing_value['goods_id']."'";
        $sum = $GLOBALS['db']->getOne($sql);
        
        $sql_goods_count = "select sum(goods_number)  from " . $ecs->table('goods') ." where goods_id = '".$costing_value['goods_id']."'";
        $sum_goods = $GLOBALS['db']->getOne($sql_goods_count);
        if($sum != $sum_goods)  //判断库存数量是否相等，不相等有出入，订单不能进行发货处理
        {
            $GLOBALS['db']->rollback();
            $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
            sys_msg($_LANG['act_false']."--商品的库存跟采购库存不一样，请检查好再进行发货", 1, $links);
        }
        if($costing_value['goods_number'] > $sum) //判断库存数量跟购买的数量比较，购买的数量大于库存数量，不能进行发货处理
        {
            $GLOBALS['db']->rollback();
            $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
            sys_msg($_LANG['act_false']."--商品的库存数量不足以满足发货的数量，请检查好再进行发货", 1, $links);
        }
        $stock_type = -1 ;    //商品入库处理， 默认为 1（增加）  -1（减少）
        $stock_status = -1;   //1:添加入库产品，-1：发货时候库存减少状态，3：库存不够的时候，4 退货的时候库存会增加状态）
        $goods_number = $costing_value['goods_number'];   //总的购物商品数量
        $order_number = $GLOBALS['db']->getAll("select id, goods_number, costing_price  from " . $ecs->table('stock_control') ." where goods_id = '".$costing_value['goods_id']."' and goods_number !=0 order by log_time ");
        $stock_costing_price = 0 ;  // 定义商品的初始成本价格为0 每次商品的循环重新设定价格为0进行计算
        foreach($order_number as $number_key=>$number_value)
        {
            $goods_number_new = 0;
            $goods_number_new = $goods_number_new + $number_value['goods_number'];
            $goods_number_count = $goods_number_new - $goods_number;                                  
            $goods_number_n = $number_value['goods_number'] - $goods_number; 
            if($goods_number_count > 0 )
            {
                $result_gn = $GLOBALS['db']->query("UPDATE " . $ecs->table('stock_control') . " SET goods_number = '". $goods_number_n ."' WHERE id= '".$number_value['id']."'"); 
                if(!$result_gn)
                {
                    $GLOBALS['db']->rollback();  //事务中断
                    $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
                    sys_msg($_LANG['act_false']."--发货的时候，库存出现问题,请认真检查", 1, $links);  
                }
                
                //写入相关的入库数据记录
                $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, ".
                             "  goods_number, stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id )  ".
                           " VALUES ('" . $number_value['id'] . "', '".$costing_value['goods_name']."', '".gmtime()."', '". 
                             $goods_number ."', '". $stock_type ."', '".$number_value['costing_price']."', '" . $order_sn . "' ,  '".
                             $stock_status."', '".real_ip()."','". real_ip() ."', '$admin_agency_id')";
                $GLOBALS['db']->query($sql_log);
                
                $stock_costing_price = $stock_costing_price + $goods_number * $number_value['costing_price']; 
                               
                break; // 终止循环  
            }
            else 
            {
                $goods_number = $goods_number - $goods_number_new ; 
                $result_gn_1 = $GLOBALS['db']->query("UPDATE " . $ecs->table('stock_control') . " SET goods_number = 0 WHERE id= '".$number_value['id']."'");               
                if(!$result_gn_1)
                {
                    $GLOBALS['db']->rollback();  //事务中断
                    $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
                ...
                $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, ".
                             " goods_number, stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id )  ".
                          " VALUES ('" . $number_value['id'] . "', '".$costing_value['goods_name']."', '".gmtime()."', '". 
                             $number_value['goods_number'] ."', '". $stock_type ."', '".$number_value['costing_price']."', '" . 
                             $order_sn . "' ,  '".$stock_status."', '".real_ip()."','". real_ip() ."' , '$admin_agency_id')";
                $GLOBALS['db']->query($sql_log);
                $stock_costing_price = $stock_costing_price + $number_value['goods_number'] * $number_value['costing_price'] ; 
            }
        }
        $costing_price_sql = "UPDATE " . $GLOBALS['ecs']->table('order_goods') . 
                        " SET stock_costing_price = '" . $stock_costing_price ."' " .
                        " WHERE rec_id = " . $costing_value['rec_id'] ;
        $result_gn_2 = $GLOBALS['db']->query($costing_price_sql);
        if(!$result_gn_2)
        {
            $GLOBALS['db']->rollback();  //事务中断
            $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
            sys_msg($_LANG['act_false']."--发货的时候，库存出现问题,更新商品成本出错了", 1, $links);  
        }
        
        //$goods_number_up = $sum_goods - $costing_value['goods_number'];
        //$GLOBALS['db']->query("UPDATE " . $ecs->table('goods') . " SET goods_number = '".$goods_number_up."'  WHERE goods_id = '".$costing_value['goods_id']."'");
    } 
    ...
    /* 检查此单发货商品库存缺货情况 */
    $virtual_goods = array();
    $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, SUM(DG.send_number) AS sums, IF(DG.product_id > 0, P.product_number, G.goods_number) AS storage, G.goods_name,G.host_goods_id, DG.send_number
	...
	/*foreach($delivery_stock_result as $host_k=>$host_v){
		if($host_v['host_goods_id'])
		{
			$host_goods_number =  $GLOBALS['db']->getOne("select goods_number from ". $GLOBALS['ecs']->table('goods') . " where goods_id = '$host_v[host_goods_id]'") ;
			$delivery_stock_result[$host_k]['goods_number'] = $host_goods_number;
			$delivery_stock_result[$host_k]['goods_id'] = $host_v['host_goods_id'];
    ...
    {
        $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, SUM(DG.send_number) AS sums, G.goods_number,G.host_goods_id, G.goods_name, DG.send_number
        FROM " . $GLOBALS['ecs']->table('delivery_goods') . " AS DG, " . $GLOBALS['ecs']->table('goods') . " AS G
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
        GROUP BY DG.goods_id ";
        $delivery_stock_result = $GLOBALS['db']->getAll($delivery_stock_sql);
		/*add by hg for date 2014-04-25 获取实际库存 begin*/
		/* foreach($delivery_stock_result as $host_k=>$host_v){
    ...
    /*ccx 2014-12-29 start 因为要结合新建的库存表数据信息, 后台发货的时候都要减库存(就算在后台控制使用了关闭库存管理的功能)*/
    else 
    {
       foreach ($delivery_stock_result as $value)
        {
    ...
    if ($order_finish)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            /* 发放红包 */
            /*  2014/12/22 18:03:12 圣诞活动结束后在开启 weichen
            //send_order_bonus($order_id);
            */
            
            /* 针对预售商品发放红包 */
            /*  2015-03-09 针对预售荔枝送蜂蜜送代金券(后台的红包) 开启
            */
            send_order_bonus($order_id);
        }
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '')
        {
			...
			$obj_sms = new class_sms($order['mobile'],sprintf($GLOBALS['_LANG']['order_shipped_sms'], $order['order_sn'],local_date($GLOBALS['_LANG']['sms_time_format']), $GLOBALS['_CFG']['shop_name']));
        	$obj_sms->send_sms();//发送短信	
		...
		foreach($order_res as $idx=>$val)
		{
			...
			if($sales_volume)
			$db->query("update " . $ecs->table('goods') . " set sales_volume=$sales_volume WHERE goods_id =".$val['goods_id']);
		}	
    }
    $GLOBALS['db']->commit();
    /** ccx   库存操作 结束 end **/
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        $delivery_stock_sql = "SELECT DG.goods_id, DG.product_id, DG.is_real, SUM(DG.send_number) AS sums
            ...
            GROUP BY DG.goods_id ";
        $delivery_stock_result = $GLOBALS['db']->getAll($delivery_stock_sql);
		/*add by hg for date 2014-04-25 获取实际商品id begin*/
		/*foreach($delivery_stock_result as $host_k => $host_v){
			$host_goods_id = $GLOBALS['db']->getOne("select host_goods_id from " . $GLOBALS['ecs']->table('goods') . " where goods_id=$host_v[goods_id]");
			if($host_goods_id)
			{
				$delivery_stock_result[$host_k]['goods_id'] = $host_goods_id;
			}
		}*/
		/*end*/
    ...
    /*ccx 2015-01-08 start 因为要结合新建的库存表数据信息, 后台取消发货的时候都要增加库存(就算在后台控制使用了关闭库存管理的功能)*/
    else 
    {
      // 检查此单发货商品数量
        $virtual_goods = array();
    ...
    //ccx 取消发货的时候，商品库存管理的数据也要进行变化（把退货的商品数量添加到库存表当中）
    $order_sn = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $ecs->table('order_info') . " WHERE order_id = '".$order_id."' ") ;
    //ccx 取消发货的时候，商品库存管理的数据也要进行变化（把退货的商品数量添加到库存表当中）
    $stock_control_number = $GLOBALS['db']->getAll("select id, stock_id, goods_number, goods_name, costing_price, stock_number ,admin_agency_id from " . $ecs->table('stock_control_log') ." where stock_number = '$order_sn' and stock_type = '-1' and  stock_status = '-1'");
    if(count($stock_control_number) < count($delivery_stock_result))  // 判断日志记录是否小于delivery_goods这个表产品的记录数，如果小于，中断事务，提示库存日志跟产品有错误
    {
        $GLOBALS['db']->rollback();
        $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
        sys_msg($_LANG['act_false']."--库存缺少相关数据记录，请检查好相关库存日志是否有手动删除的操作", 1, $links);
    }
    
    foreach($stock_control_number as $stock_key=>$stock_value)
    {  
        //取消发货，记录相关的库存回滚的记录
        $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, goods_number, stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id, parent_id )  ".
                  " VALUES ('" . $stock_value['stock_id'] . "', '".$stock_value['goods_name']."', '".gmtime()."', '". $stock_value['goods_number'] ."', '1', '".$stock_value['costing_price']."', '" . $stock_value['stock_number'] . "' ,  '4', '".real_ip()."','". real_ip() ."' , '".$stock_value['admin_agency_id']."', '".$stock_value['id']."')";
        ...
        $stock_number = "UPDATE " . $GLOBALS['ecs']->table('stock_control') . 
                        " SET goods_number = goods_number + " . $stock_value['goods_number'] . 
                        " WHERE id = " . $stock_value['stock_id'] . " and costing_price = '".$stock_value['costing_price']."' and admin_agency_id = ".$stock_value['admin_agency_id']."";
        $result_number = $GLOBALS['db']->query($stock_number);
        if(!$result_number || $GLOBALS['db']->affected_rows() != 1)
        {
            $GLOBALS['db']->rollback();  //事务中断
            $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
            sys_msg($_LANG['act_false']."--库存缺少相关数据记录，请检查好相关库存日志是否有手动删除的操作", 1, $links);
    ...
    $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('stock_control_log') . " SET stock_status = '5' WHERE stock_number ='". $order_sn ."' and stock_type ='-1' and stock_status = '-1'");
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            ...
            "WHERE o.user_id > 0 " .
            "AND o.extension_code = '' " . order_query_sql('unprocessed').$where;
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['ship']))
    {
        ...
        if ($goods_list)
        {
            ...
            {
				/*add by hg for date 2014-04-30取主站商品库存*/
				/*add by weichen 2014-12-04 取消获取主站的库存
				if(!empty($goods_value['host_goods_id']) && empty($goods_value['product_id']))
				{
					$goods_value['storage'] = $goods_list[$key]['storage'] = $db->getOne("select goods_number from ".$ecs->table('goods')." where goods_id=$goods_value[host_goods_id]");
				}
				*/
    ...
    {
		/* 检查权限 */
		admin_priv('order_del');
		check_authz_json('order_del');
        $require_note = false;
        $operation = 'remove';
        if (!$batch)
        {
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('split' == $operation)
    {
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            {
				/*add by hg for date 2014-04-25 获取实际商品id*/
				if($value['host_goods_id'])
				{
					 $value['goods_id'] = $value['host_goods_id'];
				}
				/*end*/
                //如果是货品则键值为商品ID与货品ID的组合
                $_key = empty($value['product_id']) ? $value['goods_id'] : ($value['goods_id'] . '_' . $value['product_id']);
		...
		/*add by hg for date 2014-04-22 写入代理商id*/
		$_delivery['admin_agency_id'] = $order['admin_agency_id'];
        /* 发货单入库 */
    ...
    elseif ('unship' == $operation)
    {
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
        {
			...
			/* ccx 2014-12-08 当订单有发货修改为未发货状态的时候， 商品的库存数量需要回滚  start */
			$return_message =  change_stock_control_message($order['order_id'],$operation);
			if($return_message == false)
			{
			    /* 操作失败 */
                $links[] = array('text' => "设置成为未发货出现问题，请认真检查", 'href' => 'order.php?act=info&order_id=' . $order_id);
                sys_msg($_LANG['act_false']."--设置成为未发货出现问题，请认真检查--", 1, $links);
			}
    		/** ccx   库存操作 结束 end **
			/* ccx 2014-12-08 商品的库存数量需要回滚  end   */  
			
			/* ccx 2014-12-08 当订单有发货修改为未发货状态的时候， 商品的库存数量需要回滚  start */
			/*
			$order_sn = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $ecs->table('order_info') . " WHERE order_id = '".$order['order_id']."' ") ;
            //ccx 2014-12-08 设置为未发货处理的时候，商品库存管理的数据也要进行变化（把发货的商品数量重新回滚到库存表当中）
            $stock_control_number = $GLOBALS['db']->getAll("select id, stock_id, goods_number, goods_name, costing_price, stock_number ,admin_agency_id from " . $ecs->table('stock_control_log') ." where stock_number = '$order_sn' and stock_type = '-1' and  stock_status = '-1'");
            foreach($stock_control_number as $stock_key=>$stock_value)
            {  
                //退货，记录相关的库存回滚的记录
                $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, goods_number, stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id, parent_id )  VALUES ('" . $stock_value['stock_id'] . "', '".$stock_value['goods_name']."', '".gmtime()."', '". $stock_value['goods_number'] ."', '1', '".$stock_value['costing_price']."', '" . $stock_value['stock_number'] . "' ,  '6', '".real_ip()."','". real_ip() ."' , '".$stock_value['admin_agency_id']."', '".$stock_value['id']."')";
                $GLOBALS['db']->query($sql_log);
                
                $stock_number = "UPDATE " . $GLOBALS['ecs']->table('stock_control') . "SET goods_number = goods_number + " . $stock_value['goods_number'] . " WHERE id = " . $stock_value['stock_id'] . " and costing_price = '".$stock_value['costing_price']."' and admin_agency_id = '".$stock_value['admin_agency_id']."'";
                $GLOBALS['db']->query($stock_number);
            }
            $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('stock_control_log') . " SET stock_status = '7' WHERE stock_number ='". $order_sn ."' and stock_type ='-1' and stock_status = '-1'");
			*/
			/* ccx 2014-12-08 商品的库存数量需要回滚  end   */  
        }
        
        /*ccx 2014-01-08 start 因为要结合新建的库存表数据信息, 后台设置未发货的时候库存需要回滚(就算在后台控制使用了关闭库存管理的功能)*/
        else
        {
            change_order_goods_storage($order['order_id'], false, SDT_SHIP);
            
            /* ccx 2015-01-08 当订单有发货修改为未发货状态的时候， 商品的库存数量需要回滚  start */
            $return_message =  change_stock_control_message($order['order_id'],$operation);
            if($return_message == false)
            {
                /* 操作失败 */
    ...
    elseif ('return' == $operation)
    {
        ...
        if ($_CFG['use_storage'] == '1')
        {
            ...
            /*ccx 2014-12-08 减库存数量 start   */ 
            $order_sn = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $ecs->table('order_info') . " WHERE order_id = '".$order['order_id']."' ") ;
            //ccx 2014-12-08 退货处理的时候，商品库存管理的数据也要进行变化（把退货的商品数量添加到库存表当中）
            $stock_control_number = $GLOBALS['db']->getAll("select id, stock_id, goods_number, goods_name, costing_price, stock_number ,admin_agency_id from " . $ecs->table('stock_control_log') ." where stock_number = '$order_sn' and stock_type = '-1' and  stock_status = '-1'");
            foreach($stock_control_number as $stock_key=>$stock_value)
            {   
                                
                //退货，记录相关的库存回滚的记录
                $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, goods_number," . 
                           " stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id, parent_id )  ".
                " VALUES ('" . $stock_value['stock_id'] . "', '".$stock_value['goods_name']."', '".gmtime()."', '". $stock_value['goods_number'] ."', '1', '".$stock_value['costing_price']."', '" . $stock_value['stock_number'] . "' ,  '6', '".real_ip()."','". real_ip() ."' , '".$stock_value['admin_agency_id']."', '".$stock_value['id']."')";
                ...
                if(!$result_ok)
				{
					$GLOBALS['db']->rollback();  //事务中断
					$links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
                    sys_msg($_LANG['act_false']."--退货操作出现异常,请返回检查相应的库存记录", 1, $links);
				}
                $stock_number = "UPDATE " . $GLOBALS['ecs']->table('stock_control') . 
                                " SET goods_number = goods_number + " . $stock_value['goods_number'] . 
                                " WHERE id = " . $stock_value['stock_id'] . 
                                " and costing_price = '".$stock_value['costing_price']."' and admin_agency_id = ".$stock_value['admin_agency_id']."";
                $result_number = $GLOBALS['db']->query($stock_number);
				if(!$result_number || $GLOBALS['db']->affected_rows() != 1)
                {
                    $GLOBALS['db']->rollback();  //事务中断
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back .= " ( '" . $list['delivery_sn'] . "', '" . $list['order_sn'] . "',
                              ...
                              '" . $list['agency_id'] . "', '" . $list['invoice_no'] . "', '" . $list['admin_agency_id'] . "'
                              )";
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
		...
		if(!if_agency())
		{
			...
			if(!empty($top_rank)) $where = "top_rank = $top_rank AND ";
		}
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['ecs']->table('users') .
                " WHERE $where (user_id LIKE '%" . mysql_like_quote($id_name) . "%'" .
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
		/* 加入商品归属条件 */
		$where = agency_where();
		if(!$where) $where = " AND admin_agency_id = 0";
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['ecs']->table('goods') .
                ...
                " OR goods_sn LIKE '%" . mysql_like_quote($keyword) . "%') $where" .
                " LIMIT 20";
        $res = $GLOBALS['db']->query($sql);
...
function operable_list($order)
{
    ...
    /* 取得订单支付方式是否货到付款 */
    $payment = payment_info($order['pay_id']);
    $is_cod  = $order['pay_num'] == 2?$order['pay_num']:'';
...
function order_list()
{
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $sql_luck = "SELECT id FROM " . $GLOBALS['ecs']->table('lucky_draw') .
...
function get_order_goods($order)
{
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*,g.host_goods_id, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, p.product_id,g.host_goods_id,o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
...
function overall_order_amount($sql)
{
	...
	$row = $GLOBALS['db']->getAll($sql);
	$arr = array('overall_formated_total_fee'=>'','overall_formated_order_amount'=>'');
	if(!empty($row))
	foreach($row as $k=>$v){
		$arr['overall_formated_total_fee'] += $v['total_fee'];
		$arr['overall_formated_order_amount'] += $v['order_amount'];
	}
	return $arr;
}

/*
ccx 2014-12-08 当订单的状态发生变化的时候，如果使用了库存，并且库存改变过的，库存表的数据记录也要相应的发生变化
*/
function change_stock_control_message($order_id, $operation )
{
	    if('unship' == $operation)  //设置为未发货
		{
			$stock_status_new = 8 ; //新日志的数据的写入状态变为8： 重新设置未发货的操作，所产生的商品的库存回滚数量的状态
			$stock_status_old = 9;  //把该订单的去发货记录的状态更改为9： 表示该条日志记录已经被设置为未发货的状态
		}    
	    /* ccx 2014-12-08 当订单有发货修改为未发货状态的时候， 商品的库存数量需要回滚  start */
		$order_sn = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = '".$order_id."' ") ;
		//ccx 2014-12-08 设置为未发货处理的时候，商品库存管理的数据也要进行变化（把发货的商品数量重新回滚到库存表当中）
		$stock_control_number = $GLOBALS['db']->getAll("select id, stock_id, goods_number, goods_name, costing_price, stock_number ,admin_agency_id from " . $GLOBALS['ecs']->table('stock_control_log') ." where stock_number = '$order_sn' and stock_type = '-1' and  stock_status = '-1'");
        foreach($stock_control_number as $stock_key=>$stock_value)
        {  
        	//退货，记录相关的库存回滚的记录
            $sql_log = "INSERT INTO " . $GLOBALS['ecs']->table('stock_control_log') . " (stock_id, goods_name, log_time, goods_number, stock_type, costing_price, stock_number, stock_status, stock_note, ip_address, admin_agency_id, parent_id ) ".
            " VALUES (" . $stock_value['stock_id'] . ", '".$stock_value['goods_name']."', '".gmtime()."', ". $stock_value['goods_number'] .", 1, '".$stock_value['costing_price']."', '" . $stock_value['stock_number'] . "' ,  '".$stock_status_new."', '".real_ip()."','". real_ip() ."' , ".$stock_value['admin_agency_id'].", ".$stock_value['id'].")";
            $result_ok = $GLOBALS['db']->query($sql_log);
            if(!$result_ok)
            {
                $GLOBALS['db']->rollback();  //事务中断
        ...
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('stock_control_log') . " SET stock_status = '".$stock_status_old."' WHERE stock_number ='". $order_sn ."' and stock_type ='-1' and stock_status = '-1'");
        /* ccx 2014-12-08 商品的库存数量需要回滚  end   */  
        return true;
}
?>