...
/*------------------------------------------------------ */
//-- ccx 2014-12-26  自定义导航栏列表查询主站的 Ajax 
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'host_navigator_query')
{
    $navdb = get_host_nav();
    //print_r($navdb);exit
    $smarty->assign('agency_list',   agency_list());
    ...
    make_json_result($smarty->fetch('host_navigator.htm'), '', array('filter' => $navdb['filter'], 'page_count' => $navdb['page_count']));
}
/* ccx 2014-12-26 结束*/
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        /*ccx 2014-12-24 自定义导航栏增加代理商自己去定义*/
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                /* ccx 2014-12-24 修改代理商也能够添加自定义导航栏
                ...
                */
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . 
                           " (name,ctype,cid,ifshow,vieworder,opennew,url,type,admin_agency_id) ".
                       "VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder',".
                           " '$item_opennew','$item_url','$item_type', ".admin_agency_id().")";
            }
        }
        ...
        if(empty($sql))
        {
            ...
            */
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . 
                     " (name,ifshow,vieworder,opennew,url,type, admin_agency_id) ".
                   " VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type', ".admin_agency_id().")";
        }
        $db->query($sql);
        clear_cache_files();
...
/*--------------------------------------------------------------------
//-----ccx 2014-12-25 开始 显示主站的自定义的导航栏目 提供给代理商进行选择 
-----------------------------------------------------------------------*/

elseif($_REQUEST['act'] == 'host_navigator')
{
    $smarty->assign('agency_list',   agency_list());
    $action_list = if_agency()?'all':'';
    $smarty->assign('all',         $action_list);
    ...
    assign_query_info();
    $smarty->display('host_navigator.htm');
}

/*ccx 2014-12-25 结束*/

/*--------------------------------------------------------------------
//-----ccx 2014-12-26 开始 显示主站的自定义的导航栏目 提供给代理商进行编辑并且添加 
-----------------------------------------------------------------------*/
elseif($_REQUEST['act'] == 'copyhost')
{
    $id = $_REQUEST['id'];
    if (empty($_REQUEST['step']))
    {
        $rt = array('act'=>'add_host','id'=>$id);
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id='$id'");
        $rt['item_name'] = $row['name'];
        $rt['item_url'] = $row['url'];
        $rt['item_vieworder'] = $row['vieworder'];
...
/* --------------------------------
// ---- 2014-12-26 ccx 引用并且添加主站的导航栏为自己使用
-------------------------------- */
elseif($_REQUEST['act'] == 'add_host')
{
   //echo "chenchoaxiang";exit; 
    $host_id = $_REQUEST['id'];
    $item_name = $_REQUEST['item_name'];
    $item_url = $_REQUEST['item_url'];
    $item_ifshow = $_REQUEST['item_ifshow'];
    ...
    if($item_ifshow == 1 && $item_type == 'middle')
    {
        ...
        if($arr)
        {
            ...
            */
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . 
                       " (name,ctype,cid,ifshow,vieworder,opennew,url,type,admin_agency_id, host_id ) ".
                   "VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder',".
                       " '$item_opennew','$item_url','$item_type', ".admin_agency_id().", '$host_id' )";
        }
    }
    ...
    if(empty($sql))
    {
        ...
        */
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . 
                 " (name,ifshow,vieworder,opennew,url,type, admin_agency_id, host_id ) ".
               " VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type', ".admin_agency_id()." , '$host_id')";
    }
    $db->query($sql);
    clear_cache_files();
    $links[] = array('text' => $_LANG['navigator'], 'href' => 'navigator.php?act=list');
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('nav'). $where;
        //echo $sql;
...
/*ccx 2014-12-25 开始 在代理商后台能够获取到主站的导航信息 */
function get_host_nav()
{
    ...
    if($result === false)
    {
        ...
        */
        ...
        $admin_agency_id = admin_agency_id();
        if($admin_agency_id)
        $where .=" AND (SELECT id FROM ".$GLOBALS['ecs']->table('nav').
                       " WHERE admin_agency_id = $admin_agency_id and host_id = n.id limit 1) is null";
        ...
        /* 查询 */
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type".
               " FROM ".$GLOBALS['ecs']->table('nav').' as n '. $where.
               " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
               " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
...
/*ccx 2014-12-25 结束 在代理商后台能够获取到主站的导航信息 */
?>
