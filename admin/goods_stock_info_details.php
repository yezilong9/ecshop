...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('goods_stock_info_details.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('goods_stock_info_details.htm'), '',
...
function get_stock_control_log()
{
    ...
    if ($result === false)
    {
        ...
        $filter['start_date'] = empty($_REQUEST['start_date']) ? local_strtotime('-365 days') : $_REQUEST['start_date'];
	...
	$current_order_amount = current_order_amount($sql); // 统计所采购商品的价格总计和数量总计
    ...
    $GLOBALS['smarty']->assign('keyword',          $filter['keyword']);
...
function current_order_amount($sql)
{
