...
if ($_REQUEST['act'] == 'list')
{
	...
	/*add by hg for date 2014-04-1 代理商列表与当前选择代理商 begin*/
	$agency_user_id = $_GET['agency']?$_GET['agency']:0;
	$agency_tpl = agency_templates($agency_user_id);
	$curr_template = $agency_tpl['tpl_name'];
	$curr_style = $agency_tpl['tpl_fg']; 
	//dump($agency_tpl['res']);
	$smarty->assign('agency_res',$agency_tpl['res']);
	$smarty->assign('agency_user_id',$agency_tpl['agency_user_id']);
	$smarty->assign('agency_user_name',$agency_tpl['agency_user_name']);
	/*end*/
    /* 获得当前的模版的信息 */
	if(!$curr_template)
	{
...
if ($_REQUEST['act'] == 'setup')
{
	...
	/*add by hg for date 2014-04-01 获取代理商信息*/
	$agency_user_id = $_REQUEST['agency']?$_REQUEST['agency']:0;
	$agency_tpl = agency_templates($agency_user_id);
	//print_r($agency_user_id);die;
	$template_theme = $agency_tpl['tpl_name'];
	$smarty->assign('agency_res',$agency_tpl['res']);
	$smarty->assign('agency_user_name',$agency_tpl['agency_user_name']);
	$smarty->assign('agency_user_id',$agency_tpl['agency_user_id']);
	$smarty->assign('template_theme',$template_theme);
	/*end*/
...
{
    admin_priv('template_setup');
	$curr_template = $_POST['agency_tpl'];
	if(!$curr_template){
		$curr_template = $_CFG['template'];
	}
    ...
    if (file_put_contents($template_file, $template_content))
    {
        ...
        clear_cache_files();
        $lnk[] = array('text' => $_LANG['go_back'], 'href'=>'template.php?act=setup&template_file=' .$_POST['template_file'].'&agency=' .$_POST['agency']);
...
if ($_REQUEST['act'] == 'install')
{
	...
	$user_id = $_GET['user_id']?trim($_GET['user_id']):0;
	/*add by hg for date 2014-04-1 设置代理商的模板与风格 bagen*/

	if($user_id)
	{
		$tplArr = array('tpl_name'=>$tpl_name,'tpl_fg'=>$tpl_fg);
		$tplArr = serialize($tplArr);
		$tpl_static = $db->query("UPDATE " . $ecs->table('admin_user') . "SET agency_template = '$tplArr' WHERE user_id = $user_id");
	}else{
		...
		$tpl_static = $db->query($sql, 'SILENT');
	}
 ...
 *
**/
function agency_templates($id=''){

	$agency_user_id = $id?$id:0;

	$agency_res = $GLOBALS['db']->getAll("select user_id,user_name from " .$GLOBALS['ecs']->table('admin_user')." where agency_user_id != '' or action_list = 'all'");
	

	
	foreach($agency_res as $key=>$value){
		if($value['user_id'] == $agency_user_id)
		{
			$agency_user_name = $value['user_name'];
		}
	}
	$res_tpl = $GLOBALS['db']->getRow("select agency_template from " .$GLOBALS['ecs']->table('admin_user') ."where user_id = $agency_user_id");
	$agency_tpl = unserialize($res_tpl['agency_template']);

	$agency_tpl['res'] = $agency_res;
	$agency_tpl['agency_user_id'] = $agency_user_id;
	$agency_tpl['agency_user_name'] = $agency_user_name;
	return $agency_tpl;exit;
