...
if ($_REQUEST['act'] == 'list')
{
    ...
    $account['voucher_amount'] = get_total_amount($start_date, $end_date,'0',$admin_agency_id);//充值总额
    $account['to_cash_amount'] = get_total_amount($start_date, $end_date, 1,$admin_agency_id);//提现总额

    $sql = " SELECT IFNULL(SUM(a.user_money), 0) AS user_money, IFNULL(SUM(a.frozen_money), 0) AS frozen_money FROM " .
           $ecs->table('account_log') . "As a left join ".$ecs->table('users')."As u on a.user_id = u.user_id  WHERE `change_time` >= " . $start_date ." AND `change_time` < " .($end_date+86400)." $where";
    ...
    $sql = "SELECT IFNULL(SUM(surplus), 0) AS surplus, IFNULL(SUM(integral_money), 0) AS integral_money FROM ".
           $ecs->table('order_info') ." WHERE 1 AND `add_time` >= " . $start_date ." AND `add_time` < " .($end_date+86400)."   $agency_where";
    $money_list = $db->getRow($sql);
...
function get_total_amount ($start_date, $end_date, $type=0,$admin_agency_id)
{
    ...
    $sql = " SELECT IFNULL(SUM(amount), 0) AS total_amount FROM " . $GLOBALS['ecs']->table('user_account') . " AS a, " . $GLOBALS['ecs']->table('users') . " AS u ".
       " WHERE process_type = $type AND is_paid = 1 AND a.user_id = u.user_id AND paid_time >= '$start_date' AND paid_time < '" . ($end_date + 86400) . "' $where";
