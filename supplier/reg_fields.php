...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO " .$ecs->table('reg_fields') ."( ".
                "reg_field_name, dis_order, display, is_need,upload".
            ") VALUES (".
                "'$_POST[reg_field_name]', '$_POST[reg_field_order]', '$_POST[reg_field_display]', '$_POST[reg_field_need]', '$_POST[reg_field_upload]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT id AS reg_field_id, reg_field_name, dis_order AS reg_field_order, display AS reg_field_display, is_need AS reg_field_need,upload FROM ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$ecs->table('reg_fields') . " SET `reg_field_name` = '$_POST[reg_field_name]', `dis_order` = '$_POST[reg_field_order]', `display` = '$_POST[reg_field_display]', `is_need` = '$_POST[reg_field_need]', `upload` = '$_POST[reg_field_upload]' WHERE `id` = '$_POST[id]'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	/* add by hg for date 2014-03-27 上传证照和代理商地址不可删除*/
	if($field_id == 100 || $field_id ==101)
	{
		$field_id = 0;
	}
	/*end*/
