...
if ($_REQUEST['act'] == 'list')
{
    ...
    /*获得当前配送方式对应地区的基本配送运费 add by zenghd 2014-09-15 17:40*/
    foreach($list as $l_k=>$l_v){
    	$shipping_cfg = unserialize_config($l_v['configure']);
    	$list[$l_k]['base_fee'] = $shipping_cfg['base_fee'];
    	//print_r($shipping_cfg);
    }
    ...
    $smarty->assign('ur_here',  '<a href="shipping.php?act=list">'.
        $_LANG['03_shipping_list'].'</a> - ' . $_LANG['shipping_area_list'] . "及基本运费列表 <b style='font-weight:normal;color:red;'> (请点击右侧 新建运费模板 添加地区及运费。)</b>");
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))// 
{
    ...
    $smarty->assign('ur_here',          $shipping['shipping_name'] .' - '.'新建运费模板（配送地区及运费）');
...
{	
    ...
    else
    {
        ...
        /* 过滤重复选择的地区 add by zenghd 2014-09-15 17:40*/
        $shipping_id = intval($_REQUEST['shipping']);
		$region_id_list=get_region_id_list($shipping_id);
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                if(in_array($val, $region_id_list))continue;
...
//       }
    ...
    assign_query_info();
    $smarty->assign('ur_here',          $row['shipping_name'] .' - '. $_LANG['edit_area'].'及运费');
...
{//echo $_REQUEST['act'];exit;
    ...
    else
    {
        ...
        {
            $region_list[$row['region_id']] = $row['parent_id'];//$region_list[33]] = 1
        }
        ...
        /* 添加选定的城市和地区 并过滤重复选择的地区 add by zenghd 2014-09-15 19:40*/
        $shipping_id = intval($_REQUEST['shipping']);
        $region_id_list=get_region_id_list($shipping_id);
        //dump($region_id_list);
 ...
 * @param   int     $shipping_id    配送id
 */
function get_region_id_list($shipping_id)
{
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('shipping_area');
	if ($shipping_id > 0)
	{
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{   
		$sql = "SELECT region_id FROM " . $GLOBALS['ecs']->table('area_region').
				"WHERE shipping_area_id = '$row[shipping_area_id]'";
		$rows= $GLOBALS['db']->getAll($sql);
		foreach($rows as $r_k=>$r_v){
			$region_id_list[] = $r_v['region_id'];
		}
	}

	return array_unique($region_id_list);
}
