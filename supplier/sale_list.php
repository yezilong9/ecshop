...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        foreach ($goods_sales_list['sale_list_data'] AS $key => $value)
        {
            ...
            echo ecs_iconv(EC_CHARSET, 'GB2312', $value['costing_price']) . "\t";
...
function get_sale_list($is_pagination = true){
    ...
    $where = " WHERE og.order_id = oi.order_id". order_query_sql('finished', 'oi.') .
             " AND oi.add_time >= '".$filter['start_date']."' AND oi.add_time < '" . ($filter['end_date'] + 86400) . "'$sale_where";
    ...
    {
		$sale_list_data[$key]['profit_total'] = price_format(($sale_list_data[$key]['sales_price'] - $sale_list_data[$key]['costing_price'])*$sale_list_data[$key]['goods_num']);
        $sale_list_data[$key]['sales_price'] = price_format($sale_list_data[$key]['sales_price']);
        $sale_list_data[$key]['costing_price'] = price_format($sale_list_data[$key]['costing_price']);
...
function statistics($sql)
{
    ...
    foreach ($sale_list_data as $key => $item)
    {
		...
		$statistics['profit_total']   += $sale_list_data[$key]['profit_total']; 
