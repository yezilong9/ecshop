...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	/* edit by ccx for date 2014-11-13  读取相应供货商的产品信息 begin*/
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'special_update')
{
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods_supplier') .
    ...
    {
        $max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$ecs->table('goods_supplier')) : $_REQUEST['goods_id'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods_supplier') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral,is_special, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,admin_agency_id,wholesale_price,costing_price,start_num, goods_shipai,province_id,province_name,city_id,city_name,area_id,area_name, supply_sign_id )" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral','$is_special', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$admin_agency_id','$wholesale_price','$costing_price','$start_num', '$_POST[goods_shipai]',$province_id,'$province_name',$city_id,'$city_name',$area_id,'$area_name', '$_SESSION[admin_id]' )";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods_supplier') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral,is_special, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,admin_agency_id,  goods_shipai, supply_sign_id )" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$admin_agency_id', '$_POST[goods_shipai]', '$_SESSION[admin_id]' )";
        }
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods_supplier') . " SET " .
	...
	{
		$goods_img_res = $db->getRow("select goods_thumb,goods_img,original_img from " . $ecs->table('goods_supplier') ." where goods_id = $copy_goods_id");
		$db->query("UPDATE " . $ecs->table('goods_supplier') . " SET goods_thumb = '$goods_img_res[goods_thumb]',goods_img = '$goods_img_res[goods_img]',original_img = '$goods_img_res[original_img]',host_goods_id='$copy_goods_id'  where goods_id = $goods_id");
		
		$goods_gallery = $db->getAll("select img_url,img_desc,thumb_url,img_original from " . $ecs->table('goods_gallery_supplier') ." where goods_id = $copy_goods_id");
		if(!empty($goods_gallery[0]))
		{
			foreach($goods_gallery as $gallery_key=>$gallery_value){
				$db->query("INSERT INTO " . $ecs->table('goods_gallery_supplier') . " (goods_id, img_url, img_desc, thumb_url, img_original)" .
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        $sql = "UPDATE " .$ecs->table('goods_supplier'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_supplier') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_supplier') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_supplier') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('goods_gallery_supplier') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $ecs->table('goods_gallery_supplier') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
    ...
    {
        $db->query("UPDATE " . $ecs->table('goods_supplier') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
        $db->query("UPDATE " . $ecs->table('goods_gallery_supplier') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
        @unlink('../' . $original_img);
        @unlink('../' . $img);
    }
	...
	$host_goods_id = $db->getAll("select goods_id from " .$ecs->table('goods_supplier'). " where host_goods_id = $goods_id");
	...
	/*end*/
	//dump($link);
    sys_msg($is_insert ? $_LANG['add_goods_ok'] : $_LANG['edit_goods_ok'], 0, $link,false);
}
    ...
    if (isset($_POST['type']))
    {
        ...
        elseif ($_POST['type'] == 'batch_shop_price')
        {
			...
			foreach($goods_id_arr as $key=>$value){
				...
				{
					if($db->query("UPDATE ".$ecs->table('goods_supplier')." SET shop_price = $shop_price_arr[$value] WHERE goods_id=$value")){
						$yes=true;
					}
					else
					{
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    $sql = "SELECT img_url, thumb_url, img_original " .
            "FROM " . $ecs->table('goods_gallery_supplier') .
            " WHERE goods_id = '$goods_id'";
    $res = $db->query($sql);
	/*add by hg for date 2014-04-21 只有管理员添加的商品才能删除图片*/
	if($goods['admin_agency_id'] == '0')
    ...
    $sql = "DELETE FROM " . $ecs->table('goods_gallery_supplier') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 删除相关表记录 */
    /*2014-11-13  ccx 供货商这边不需要关联到其他表（比如说文章，配件，关联商品这些）
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    /* 删除图片文件 */
    $sql = "SELECT img_url, thumb_url, img_original " .
            " FROM " . $GLOBALS['ecs']->table('goods_gallery_supplier') .
            " WHERE img_id = '$img_id'";
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_gallery_supplier') . " WHERE img_id = '$img_id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $ecs->table('goods_supplier') . " WHERE goods_id = '$goods_id'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $ecs->table('goods_supplier') . " WHERE goods_id = '" . $product['goods_id'] . "'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
        sys_msg($_LANG['sys']['wrong'] . $_LANG['cannot_found_goods'], 1, array(), false);
...
function agency_goods_list(){
	...
	if($admin_agency_id)
	$where .=" AND (SELECT goods_id FROM ".$GLOBALS['ecs']->table('goods_supplier').
    ...
    $sql = 'SELECT goods_id,goods_name,goods_sn,shop_price,costing_price'.
            ' FROM ' .$GLOBALS['ecs']->table('goods_supplier').' as g '.$where.' ORDER BY goods_id desc ';
...
function update_goods_stock($goods_id, $value)
{
    if ($goods_id)
    {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('goods_supplier') . "
