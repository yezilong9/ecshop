...
$sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(shipping_time), '$format') AS period, COUNT(*) AS order_count, " .
        ...
        " AND shipping_time >= '$start_time' AND shipping_time <= '$end_time' $where $agency_where" .
        " GROUP BY period ";
		//dump($sql);
...
if ($_REQUEST['act'] == 'list')
{
    ...
    array('text' => $_LANG['down_sales_stats'],
          'href'=>'sale_general.php?act=download&filename=' . $filename .
            '&query_type=' . $query_type . '&start_time=' . $start_time . '&end_time=' . $end_time. '&admin_agency_id=' . $admin_agency_id));
