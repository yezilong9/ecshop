...
var and_url = window.location.href;
var sub_url = and_url.split('message_seach');
listTable.message_url = sub_url[1];
listTable.query = "pay_message_query";
...
{
  var args = "act="+this.query+"" + this.compileFilter()+this.message_url;
  Ajax.call(this.url, args, this.listCallback, "POST", "JSON");
}
...
listTable.remove = function(id, cfm, opt)
{
  ...
  if (confirm(cfm))
  {
    var args = "act=" +  + "&id=" + id + this.compileFilter();
