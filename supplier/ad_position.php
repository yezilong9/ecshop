...
{  
	$position_name = (!empty($_REQUEST['position_name'])&&($_REQUEST['position_name']!='请输入广告位置'))?(string)$_REQUEST['position_name']: '';
    ...
    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('ad_position').$where;
    ...
    /* 查询数据 */
    $arr = array();
    $sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('ad_position').$where. ' ORDER BY position_id DESC';
