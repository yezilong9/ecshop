...
function init_rec_data()
{
	...
	special_str[0] = document.getElementById("show_special_area") == null ? '' : document.getElementById("show_special_area").innerHTML;
}

function get_cat_recommend(rec_type, cat_id)
{//console.log(hot_str[cat_id]);
    ...
    {
        if (typeof(special_str[cat_id]) == "string")
        {
            document.getElementById("show_special_area").innerHTML = special_str[cat_id];
            return;
        }
    }
...
function cat_rec_response(result)
{
    ...
    {
        var ele = document.getElementById("show_special_area");
        hot_str[res.cat_id] = res.content;
    }
