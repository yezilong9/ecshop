...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'oath' , 'oath_login', 'other_login', 'send_mobil_message', 'is_registered_ccx');
...
if ($action == 'register')
{
    if ((!isset($back_act)||empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    {
	 ...
	 if($_SESSION['user_id']){
    	echo "<script tpye='text/javascript'>alert('您已经登录了！');location.href='".$back_act."';</script>";exit;
    }
	
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $ecs->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id <> 101 AND ID <>100 ORDER BY dis_order, id';
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            /*ccx 2014-12-11 增加了提示注册成功之后送红包的提示信息*/
...
elseif($action == 'oath')
{
	...
	if($c)
    {
        ...
        if($back_act[4] != ':') $back_act = $ecs_url.$back_act;//$ecs->url()
        
		$open = empty($_REQUEST['open']) ? 0 : intval($_REQUEST['open']);
		
        $url = $c->login($ecs_url.'user.php?act=oath_login&type='.$type.'&callblock='.urlencode($back_act).'&open='.$open);
        //echo 'url:'.$url;//$ecs->url()
        //dump($c);
        if(!$url)
        {
...
elseif($action == 'oath_login')
{
    ...
    if($c)
    {
        $access = $c->getAccessToken();//return $token
        if(!$access)
        {
            show_message( $c->get_error() , '首页', $ecs->url() , 'error');
        }
        $c->setAccessToken($access);//$this->token = $token; return true;
        
		$info = $c->getMessage();//return $info
...
elseif ($action == 'logout')
{
    ...
    $user->logout();//调用的是integrate.php中的logout()
    $ucdata = empty($user->ucdata)? "" : $user->ucdata;
...
elseif ($action == 'order_detail')
{
    ...
    {
        $smarty->assign('payment_list', 1);  //修改于2014-04-03
    }
...
elseif ($action == 'act_add_booking')
{
	...
	/* 查代理商userid */
	$booking['admin_agency_id'] = $db->getOne("select admin_agency_id from ".$ecs->table('goods')." where goods_id = $booking[goods_id]");
...
elseif ($action == 'act_account')
{
    ...
    if ($surplus['process_type'] == 1)
    {
		...
		$bank_number = isset($_POST['bank_number'])?intval($_POST['bank_number']):'';
		if(!preg_match("|^[\u4e00-\u9fa5]+$|",$account_name) && !preg_match("|^[\u4e00-\u9fa5]+$|",$bank) && !preg_match("|^[0-9]+$|",$bank_number))
		{
			show_message('填写信息有误');
		}
        /* 判断是否有足够的余额的进行退款的操作 */
        ...
        if ($surplus['rec_id'] > 0)
        {
			...
			$bank_arr = array(
						...
						'user_account_id'=> $surplus['rec_id'],
						'user_id'        => $user_id,
			);
			$db->autoExecute($ecs->table('tx_bank'), $bank_arr, 'INSERT');
    ...
    else
    {
        ...
        /*add by hg for 2014-04-08*/
        $psy_res = $db->getRow("select bank_name from ". $ecs->table('pay_bank'). " where bank_code = '$surplus[payment_id]'");
        $payment_info['pay_name'] = $psy_res['bank_name'];
        ...
        $order['order_amount'] = $amount;//add by hg 2014-04-08
        //记录支付log
...
/*         /* 调用相应的支付方式文件 
        ...
        $pay_obj    = new payment();
        $order['pay_num'] = $surplus['payment_id'];
        $order['pay_num'] = $pay_obj->pay_set($order['order_sn'],$order['pay_num'],$order['order_amount'],'88');
...
elseif ($action == 'pay')
{
    ...
    //变量初始化
    $surplus_id = isset($_GET['id'])  ? intval($_GET['id'])  : 0;
    $payment_id = isset($_GET['pid']) ? $_GET['pid'] : 0;
    ...
    $payment_info['pay_name'] = $payment_info['bank_name'];
