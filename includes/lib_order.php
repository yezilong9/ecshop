...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod,a.shipping_area_id,r.region_id,a.configure ' .
...
function shipping_area_info($shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod,r.region_id,a.configure ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' .
                $GLOBALS['ecs']->table('area_region') . ' AS r ' .
    ...
    if (!empty($rows))
    {
    	...
    	foreach ($rows AS $r_key => $r_val)
    	{
    		if($r_val['region_id'] == $region_id_list['province']){
    			$shipping_province = true;//如果存在某省份的运费，把省份运费标志为true
    	...
    	foreach ($rows AS $r_key => $r_val)
    	{
    		if($shipping_province && ($r_val['region_id'] == 1))continue;
    		$row = $rows[$r_key];
    	}//过滤结束
        ...
        if (isset($shipping_config['pay_fee']))//pay_fee支付手续费
        {
...
function payment_info($pay_id)
{
	...
	/*2014-04-08 by hg*/
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('pay_bank') .
            " WHERE Local_bank_code = '$pay_id' AND enabled = 1";
    return $GLOBALS['db']->getRow($sql);
}
...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
	...
	$res = $GLOBALS['db']->getAll("select id,bank_name,bank_code,bank_img,Platform_pay from ".$GLOBALS['ecs']->table('show_bank').
	" where state = 1  
	  AND Platform_pay = 0");
	return $res;
}
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /*ccx 2015-03-02 判断该商品是否已经通过自动下架功能 start*/
    $sql = "SELECT * FROM "  
         . $GLOBALS['ecs']->table('auto_manage') . 
         " WHERE item_id = $goods_id AND type = 'goods' ";
    $autodb = $GLOBALS['db']->getRow($sql);
    //print_r($autodb);exit;
    if($autodb) //如果存在自动下架的商品，执行if
    {
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.host_goods_id,g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                "g.market_price,g.costing_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date,  g.is_promote,".
	...
	$goods['goods_number'] = host_number($goods['host_goods_id']); 暂时不需要*/
	/*end*/
    if (empty($goods))
    {
    ...
    /*ccx 2014-12-16 新增针对特定商品做限制,该商品只能允许通过手机验证的用户并且只能购买一次 开始*/
      
    if($goods['is_promote'] == 1 && $goods['is_promote'] > 0 && $goods['promote_start_date'] < gmtime() && $goods['promote_end_date'] > gmtime())
    {        
        if($_SESSION['user_id'] == 0)
        {
            $GLOBALS['err']->add("特定商品,必须要登录,而且是通过手机验证的用户才有资格购买", ERR_NOT_PROMOTE);
            return false; 
        }
        
        $sql = "SELECT phone_validate FROM " . $GLOBALS['ecs']->table('users') .
                " WHERE user_id = ".$_SESSION['user_id']; 
        $phone_validate = $GLOBALS['db']->getOne($sql);
        //print_r($phone_validate);
        if ($phone_validate ==0)
        {
            $GLOBALS['err']->add("特定商品,需要通过手机验证的用户才能进行购买", ERR_NOT_PROMOTE);
            return false;  
        }
        /*ccx 2014-12-18 开始 查看购物车内是否已经存在该特定商品,如果存在,就不能再加入购物车了*/
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
                " AND rec_type = 'CART_GENERAL_GOODS' AND (group_id='0' OR group_id = '' OR group_id IS NULL )";

        $row = $GLOBALS['db']->getRow($sql);
        if($row) //如果购物车已经有此物品，就不能再进行购买了
        {
            $GLOBALS['err']->add("您好, 您的购物车已经存在该特定商品了,每个用户只允许购买一个", 7);
            return false; 
            exit; 
        }
        
        /*ccx 2014-12-18 结束 查看购物车内是否已经存在该特定商品,如果存在,就不能再加入购物车了*/
        
        /*ccx 2014-12-18 开始 去订单商品表查找该商品是否已经购买过了,如果购买过该商品,就不能再次进行购买了*/
         $sql_order_id = "SELECT order_id FROM ". $GLOBALS['ecs']->table('order_info') .
                        "WHERE user_id = ".$_SESSION['user_id'];
         $order_id_number = $GLOBALS['db']->getAll($sql_order_id);
         if(!$order_id_number)
         {
             foreach($order_id_number as $order_key=>$order_value)
             {
                 $sql_goods_id = "SELECT count(*) FROM ". $GLOBALS['ecs']->table('order_goods') .
                                "WHERE order_id = ".$order_value['id']." AND goods_id =".$goods_id;
                 if ($GLOBALS['db']->getOne($sql_goods_id) > 0)
                 {
                    $GLOBALS['err']->add("您好, 您已经购买过该特定商品了,每个用户只允许购买一次", ERR_NOT_PROMOTE);
                    return false;  
                    break; // 终止循环  
                 }
    ...
    /*ccx 2014-12-16 新增针对特定商品做限制,该商品只能允许通过手机验证的用户并且只能购买一次 结束*/
    ...
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$spec' LIMIT 0, 1";
    ...
    */
    ...
    $parent = array(
        ...
        'costing_price' => $goods['costing_price']
    );
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    elseif (2 == $refund_type)
    {
        ...
        $account = array(
            ...
            'is_paid'      => 0,
			'admin_agency_id'=>$order['admin_agency_id']
        );
