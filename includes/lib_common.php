            ...
            foreach($res as $k=>$v)
            {
				...
				if(admin_agency_id())
				{
                    ...
                    $sql = "select cat_id from ".
                            ...
                            " where host_cat = ". $v['cat_id'] . " AND agency_cat = ',".admin_agency_id().",'";
                    $is_lead = $GLOBALS['db']->GetOne($sql);
                    if($is_lead > 0 )
                    {
                        $res[$k]['is_lead'] = 1;  //该分类被引用了
                    }
                    else 
                    {
                        $res[$k]['is_lead'] = 0;  //该分类未被引用
                    }
				}
            }
            //如果数组过大，不采用静态缓存方式
