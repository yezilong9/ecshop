...
function add_booking($booking)
{
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('booking_goods').
            " VALUES ('', '$_SESSION[user_id]', '$booking[email]', '$booking[linkman]', ".
                ...
                "'$booking[goods_amount]', '".gmtime()."', 0, '', 0, '',$booking[admin_agency_id])";
...
{
	/*add by hg for date 2014-04-24 获取代理商id*/
	$admin_agency_id = agency_user_id();
	/*end*/
	
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid, admin_agency_id)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,$admin_agency_id)";
    $GLOBALS['db']->query($sql);
