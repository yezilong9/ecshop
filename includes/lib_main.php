...
{
    $sql = 'SELECT region,id, number, type FROM ' . $GLOBALS['ecs']->table('template') .
...
function check_cat_name($res)
{
	$arr_cat = array();
	foreach($res as $k=>$v)
	{
		$arr_cat[] = $GLOBALS['db']->getOne("select cat_name from ".$GLOBALS['ecs']->table('category')." where cat_id=$v[id]");
	}
}
/**
 * 分配文章列表给smarty
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    /*ccx 2015-04-03 新增代理商和主站的logo图片显示 start */
    $smarty->assign('logo_pic',      get_logo_pic());  //自定义导航栏
    /*ccx 2015-04-02 end */
...
function set_affiliate()
{
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
        }
    }
}
...
{
    if (!empty($_COOKIE['_affiliate_uid']))
    {
        $uid = intval($_COOKIE['_affiliate_uid']);
...
function get_navigator($ctype = '', $catlist = array())
{
	...
	*/
	...
	$sql = "SELECT * FROM ". $GLOBALS['ecs']->table('nav') . 
            "WHERE ". $agency_where ." ifshow = 1 ORDER BY type, vieworder ";
	/*ccx 2014-12-26 结束  */
    $res = $GLOBALS['db']->query($sql);
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www..com/license.php?product=_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
...
/*
 ***ccx 2015-04-03 新增读取代理商和主站的logo图片  start
*/
    ...
    if($agency_id == 0)
    {
        //$shop_logo = "主站的logo";
         $sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('shop_config') . 
                " WHERE code = 'shop_logo' AND type = 'file' ";
         $row = $GLOBALS['db']->getRow($sql);
    }
    else 
    {
        //$shop_logo = "代理商的logo";
        $sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('agency_shop_config') . 
                " WHERE code = 'shop_logo' AND type = 'file' AND admin_agency_id = ".$agency_id ;
        $row = $GLOBALS['db']->getRow($sql);
    }
    return $row['value'];
}
