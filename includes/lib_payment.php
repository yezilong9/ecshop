...
function return_url($code)
{
   ...
   return $GLOBALS['ecs']->url() . 'pay_respond.php?code=' . $code[1];
   //return 'http://172.16.67.163/' . 'pay_respond.php?code=' . $code[1];
}
 ...
 * @param   string  $code   支付方式代码
 */
function BG_return_url($code)
{
	$code = explode('_',$code);
	
   return $GLOBALS['ecs']->url() . 'bg_pay_respond.php?code=' . $code[1];
	//return 'http://172.16.67.163/' . 'bg_pay_respond.php?code=' . $code[1];
}
...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            {
                /* 取得订单信息 */
                $sql = 'SELECT order_id,admin_agency_id, user_id, order_sn, consignee, address, tel, shipping_id, extension_code, extension_id, goods_amount ' .
                ...
                $GLOBALS['db']->query($sql);
				/*订单支付后扣除代理商余额*/
				if($pay_status == PS_PAYED && !empty($admin_agency_id))
				{
					agency_order_log($order_id,$admin_agency_id,$order['user_id'],$order['goods_amount']);
				}
                /* 记录订单操作记录 */
                ...
                if ($GLOBALS['_CFG']['sms_order_payed'] == '1' && $GLOBALS['_CFG']['sms_shop_mobile'] != '')
                {
					...
					$obj_sms = new class_sms($_CFG['sms_shop_mobile'],sprintf($GLOBALS['_LANG']['order_payed_sms'], $order_sn, $order['consignee'], $order['tel']));
