...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if($admin_agency_id) 
		$where = " WHERE c.agency_cat like '%,$admin_agency_id,%' OR c.host_cat = 1 ";
    else
	...
	if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.host_cat,c.agency_cat,c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show,".
 ...
 * @param   string   $market  特殊处理
 * @return  string
 */
function price_format($price, $change_price = true,$market='')
{
    if($price==='')
    {
     $price=0;
	...
	//trace($market);
	/*代理商不登陆不显示价格 by hg*/
	if((if_untx() == '1' && !$_SESSION['admin_id']) && !$market)
	{
		//return '<span style="font-size:12px; font-weight:normal;">价格登陆后可见</span>';
	}
...
function virtual_goods_ship(&$virtual_goods, &$msg, $order_sn, $return_result = false, $process = 'other')
{
    ...
    foreach ($virtual_goods AS $code => $goods_list)
    {
        ...
        if ($code == 'virtual_card')
        {
			...
			/*$virtual_card_num = count($virtual_card);
			foreach($virtual_card as $virtual_card_k =>$virtual_card_v){
				 $virtual_card[$virtual_card_k] = count($virtual_card_v['info']);
			}*/
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
	...
	/*add by hg for date 2014-04-25取得实际  begin*/
	$host_goods_id = $GLOBALS['db']->GetOne("select host_goods_id from ".$GLOBALS['ecs']->table('goods')." where goods_id = $goods[goods_id]");
	if($host_goods_id)
	{
		$goods['goods_id'] = $host_goods_id;
	}
	/*end*/
...
* 
**/
function agency_list($user='')
{
    $sql = 'SELECT agency_user_id, user_name FROM ' . $GLOBALS['ecs']->table('admin_user') .'where agency_user_id > 0';
    $res = $GLOBALS['db']->getAll($sql);
	...
	else
	{
		foreach ($res AS $k)
		{
			$agency_list[$k['agency_user_id']] = addslashes($k['user_name']);
		}
	}
    return $agency_list;
}

/*检测非法操作，代理商操作他人商品或订单等*/
function static_goods($id='',$table='goods',$field='goods_id')
{
	if(!empty($id))
	{
		$good_sql = "select admin_agency_id from " . $GLOBALS['ecs']->table($table) . "where $field = $id";
		$agency_id = $GLOBALS['db']->getOne($good_sql);
		$agency_id = $agency_id?$agency_id:0;
		
		$user_id = $GLOBALS['db']->getOne("select user_id from " . $GLOBALS['ecs']->table('admin_user') . "where agency_user_id = $agency_id");
		if($_SESSION['admin_id'] != $user_id && !if_agency()){
			die('操作非法');
		}
	}
}
...
* 
**/
function agency_user_id()
{
	$sion_user = isset($_SESSION['user_id'])?intval($_SESSION['user_id']):0;
	if(!empty($sion_user))
	{
		$agency_user_id = $GLOBALS['db']->getOne("select top_rank from ".$GLOBALS['ecs']->table('users')."where user_id = $sion_user");
	}
	return  isset($agency_user_id)?intval($agency_user_id):0;
}


/**
* 获取商品实际库存
* add by hg for date 2014-04-24
*
**/
function host_number($goods_id='0')
{
	$goods_number = $GLOBALS['db']->getOne("select goods_number from ".$GLOBALS['ecs']->table('goods')." where goods_id = $goods_id");
	return $goods_number = $goods_number?$goods_number:'0';
}

/**
* 获取所有所属这个商品的的商品id
* add by hg for date 2014
*
*/
function host($host_id='')
{
	if($host_id=='')
	{
		return false;
	...
	$goods_arr = $GLOBALS['db']->getAll("select goods_id from ".$GLOBALS['ecs']->table('goods')." where host_goods_id = $host_id");
	return $goods_arr?$goods_arr:false;
}
/**
 * 
 * 函数名称：ipCity
 ...
 * time    : add by hg for date 2014-03-26 
 */
function ipCity(){
	//获取用户IP
	...
	$time = time();
	$userip = '113.66.157.62';//广东省电信
	//$userip = '202.99.224.112'; //内蒙古网通
	//$userip = '121.193.22.45'; //内蒙古教育网
	//$userip = '221.208.11.22'; //黑龙江省联通
	//$userip = '221.208.11.22';
	//$userip = '219.131.175.26';
	//$userip="222.218.241.0";
	$url="http://ip.taobao.com/service/getIpInfo.php?ip=".$userip;
	$userip=json_decode(file_get_contents($url));	
	if((string)$userip->code=='1'){
	   return false;
 	}
 	$data = (array)$userip->data;
	//dump($data);
	return $data;
}
...
*
*/
function agency_goods()
{
	//设置默认值
	/*$site_def = '广东,广州';*/
	
	//登陆用户
	/*@$sion_user = $_SESSION['user_id'];
	if($sion_user)
	{
		$agency_list = agency_list('1');
		if(in_array($sion_user,$agency_list))
		{
			$message = $sion_user;
		}else{
			$message = agency_user_id()
		}
	}
	//非登陆用户
	/*if(!isset($message))
...
/* 		$city_res = ipCity();
		$convert = convert();
		@$city_res['region'] = $convert[$city_res['region']];
		$city_res['city'] = mb_substr($city_res['city'],0,-1);
		$site  	  = $city_res['region'].','.$city_res['city'];
		
		//print_r($site);die;
		if($site){
			$site_row = $GLOBALS['db']->getRow("select user_id from " .$GLOBALS['ecs']->table('reg_extend_info') . "where content = '$site'");
			if(!empty($site_row['user_id']))
			{
				$message = $site_row['user_id'];
				
			}else{
				
				@$city_res['city'] .= $province[$city_res['region']];
				$site = $city_res['region'].','.$city_res['city'];
				$site_row = $GLOBALS['db']->getRow("select user_id from " .$GLOBALS['ecs']->table('reg_extend_info') . "where content = '$site'");
				$message = $site_row['user_id'];
			}
		...
		if(empty($message))
		{
			$site_row = $GLOBALS['db']->getRow("select user_id from " .$GLOBALS['ecs']->table('reg_extend_info') . "where content = '$site_def'");
			$message = $site_row['user_id'];
		} */
		/*根据域名*/
		...
		if($url)
		{
			$message = $GLOBALS['db']->getOne("select agency_user_id from ".$GLOBALS['ecs']->table('agency_url')." where agency_url = '$url'");	
		}*/
		
		/*url传值*/
		/* if(isset($_GET['uid']))
		{
			$admin_agency_id = $_GET['uid'];
			setcookie("agency_uid",$admin_agency_id, time()+3600*24);
		}

		if(!isset($admin_agency_id))
		{
			$admin_agency_id = isset($_COOKIE['agency_uid'])?$_COOKIE['agency_uid']:0;
		}
		$message = $admin_agency_id; */
		//主站
	...
	/*}*/
	$message = agency_id();
	return $agency_where = "admin_agency_id = $message AND";
}
...
function if_agency()
{
	...
	$agency_user_id = $GLOBALS['db']->getOne("select agency_user_id from ".$GLOBALS['ecs']->table('admin_user')." where user_id = $id");
	return $agency_user_id?false:true;
}
...
* $agency_id 指定代理商id
**/
function agency_where($agency_id = '')
{
	$where ='';
	$admin_id = isset($_SESSION['admin_id'])?$_SESSION['admin_id']:0;
	if(!if_agency() && empty($agency_id)){
		
		$agency_user_id = $GLOBALS['db']->getOne("select agency_user_id from " . $GLOBALS['ecs']->table('admin_user') . "where user_id = $admin_id");
		
		if(empty($agency_user_id))
		{
			$agency_user_id = agency_id();
		}
		$where = " AND admin_agency_id = $agency_user_id";
	}
	elseif(!empty($agency_id))
	{
		$where = " AND admin_agency_id = $agency_id";
	}
	
	//不是代理商时返回空，是代理商时返回" AND admin_agency_id = $agency_user_id"
	return $where;
}
...
function agency_id()
{
	...
	if($url)
	{
		...
		if($url)
		{
			$agency_user_id = $GLOBALS['db']->getOne("select agency_user_id from ".$GLOBALS['ecs']->table('agency_url')." where agency_url = '$url'");	
		}
	}
	//dump($agency_user_id);
	//return $message?$message:'0';
	return $agency_user_id?$agency_user_id:'0';
}
...
/*根据管理员id获取 代理id*/
function admin_agency_id(){
	/*add by hg for date 2014-05-08 商品所属代理商ID */
	$admin_id = isset($_SESSION['admin_id'])?$_SESSION['admin_id']:0;
	$agency_user_id = $GLOBALS['db']->getOne("select agency_user_id from " . $GLOBALS['ecs']->table('admin_user') . "where user_id = $admin_id");
	return $agency_user_id?$agency_user_id:0;
}
...
* time：2014-05-11
* author：hg
* 同时满足两个条件才返回1，网址查不到，用户没有登录（这样做的原因我都忘记了）,提供check_url()自己去判断
**/
function if_untx()
{
	$message = check_url();
...
function check_url()
{
	$url = agency_url();
	if($url)
	return $GLOBALS['db']->getOne("select agency_user_id from ".$GLOBALS['ecs']->table('agency_url')." where agency_url = '$url'");
}

/**
* 生成代理商余额流水账（消费者在代理商站点购物）
* time ：2014-05-21
* author：hg
**/
function agency_order_log($order_id,$admin_agency_id,$user_id='',$goods_amount)
{
	if(!$goods_amount)
	{
		return false;
	}
	$arr['agency_order_sn'] = get_order_sn();
	$arr['order_id']        = $order_id;
	$arr['agency_id']       = $admin_agency_id;
	$arr['user_id']         = $user_id?$user_id:'0';
	$arr['order_amount']    = $goods_amount;
	$result = $GLOBALS['db']->getOne("select order_id from ".$GLOBALS['ecs']->table('agency_pay_log')." where order_id = $order_id");
	if(!$result)
	{
		do{
			$arr['agency_order_sn'] = get_order_sn(); //获取新订单号
			$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('agency_pay_log'), $arr, 'INSERT');
		...
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('agency_pay_log')." SET order_amount = order_amount + ('$goods_amount') WHERE order_id = $order_id LIMIT 1");
	}

	subtract_agency_amount($admin_agency_id,$user_id,$goods_amount,$status = '1');
}
...
* author：hg
**/
function agency_order_status($order_id,$admin_agency_id,$user_id='',$goods_amount)
{
	
	$res = $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('agency_pay_log')." SET status = 2 WHERE order_id = $order_id");
	if($res)
	subtract_agency_amount($admin_agency_id,$user_id,$goods_amount,$status = '0');
}
...
* auther：hg
**/
function subtract_agency_amount($admin_agency_id,$user_id,$goods_amount,$status = '1')
{
	
	$arr['user_id']      = $admin_agency_id;
	$arr['frozen_money'] = '0';
	$arr['rank_points'] = '0';
	$arr['pay_points'] = '0';
	...
	$user = $GLOBALS['db']->getOne("select user_name from " .$GLOBALS['ecs']->table('users'). " where user_id = $user");
	$user = $user?$user.',id:'.$user_id:'匿名用户';
	//增加
	if($status == '1')
	{
		$arr['frozen_money']   = $goods_amount;
		$arr['change_desc'] = '用户('.$user.')购物增加代理商余额';
		
		$sql = "UPDATE ".$GLOBALS['ecs']->table('users')." SET frozen_money = frozen_money + ('$arr[frozen_money]') WHERE user_id = $arr[user_id] LIMIT 1";
	}
	elseif($status == '0')
	{
		//减少
		$arr['frozen_money']   = '-'.$goods_amount;
		$arr['change_desc'] = '用户('.$user.')退货扣除代理商余额';
		$sql = "UPDATE ".$GLOBALS['ecs']->table('users')." SET frozen_money = frozen_money + ('$arr[frozen_money]') WHERE user_id = $arr[user_id] LIMIT 1";
	}
	$GLOBALS['db']->query($sql);
	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $arr, 'INSERT');
}
...
{
	//dump()
	$BankList = $GLOBALS['db']->getAll("select bank_name,bank_code,bank_img from ".$GLOBALS['ecs']->table('show_bank')." where state = 1");
	if(!$BankList) return false;
	if(!$ReturnRes)
	{
...
/*计算支付银行高度 好像只能用在一个模板上面*/
function shou_bank_height($BankList,$num=4,$one_height=50,$sum_height=150)
{
	$BankNum = ceil(count($BankList)/$num);
	
	$height  = $BankNum*$one_height >$sum_height?$sum_height:$BankNum*$one_height;
	$andHeight = $BankNum*$one_height-$height+$one_height;
	//dump($andHeight);

	$Arr['height'] = $height;
	$Arr['andHeight'] = $andHeight;
	return $Arr;
}
/**
 * 为某商品生成唯一的货号
...
/**
* 说明:优化网站用得图片地址
* add by hg for 2014-08-12
**/
function img_url()
{
	/* 获取配置图片地址 在init配置*/
	$img_arr = $GLOBALS['_CFG']['o2o_img_url'];
	$num = mt_rand(0,count($img_arr)-1);
	if(is_array($img_arr))
		$url_img = $img_arr[$num];
	return $url_img?$url_img.'/':'http://'.agency_url().'/';
}
?>