...
class integrate
{
    ...
    function login($username, $password, $remember = null)
    {
		...
		//$this->agency_user($username, $password);  检查用户登陆站点是否正确
		$user_id = $this->check_user($username, $password);
        if ($user_id > 0 && $this->agency_user($username, $password))
        {
            if ($this->need_sync)
            {
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_) && $this->is_))
        {
            /* 如果需要同步或是插件执行这部分代码 */
    ...
    function check_user($username, $password = null,$tm= '' )
    {
        ...
        if ($password === null)
        {
            ...
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE " . $this->field_name . "='" . $post_username . "' $where ";
...
*
**/
function agency_user($username, $password)
{
    //当前域名
    ...
    //域名对应代理商  主站无值
    $agency_user_id = $this->db->getOne("select agency_user_id from " . $this->table("agency_url") . "where agency_url = '$url'");
    $agency_user_id = $agency_user_id?$agency_user_id:0;
    $user_id = $this->check_user($username, $password);
    ...
    {
        //检测是否代理商
        $row = $this->db->getOne("select user_id from " . $this->table("admin_user") . "where agency_user_id = $user_id");
        $row = intval($row);
        //print_r($agency_user_id.'1');
        if(!empty($row))
            return $agency_user_id?false:true;
        $result = $this->db->getRow("select tm_mark,top_rank from " . $this->table("users") . "where user_id = $user_id");
        //非代理商登陆
        if($agency_user_id)
        {
            //return ($result['top_rank'] != $agency_user_id)?false:true;
            /*
                ccx 2014-12-10  取消账号不互通效果 主站的账号能够在代理平台登录，一个账号全站通用
            */
            return true;
        }
        else
