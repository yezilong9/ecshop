<?php
    ...
    {
        $Loaction = 'mobile/christmas_activity.php';
    ...
    if($action =='act_login')
    {
        ...
        if ($user->login($username, $password,isset($_POST['remember'])))
        {
            ...
            $smarty->assign('css_path', 'themes/' . $_CFG['template'] . '/christmas_activity/');
        ...
        else
        {
            ...
            if ($res == 0)
            {
                $smarty->assign('css_path', 'themes/' . $_CFG['template'] . '/christmas_activity/');
                $smarty->display('/christmas_activity/christmas_activity.dwt');
                //echo "<script>alert('该用户不存在,请去注册页面注册账号再进行购物,谢谢合作');</script>";
                //confirm('Delete?')
                echo "<script>if(confirm('该用户不存在,请去注册页面注册账号再进行购物,谢谢合作')){window.location.href='user.php?act=register'}</script>";
    ...
    elseif($action =='addtocar')
    {
        ...
        if($goods_id == 10721 || $goods_id == 10783 || $goods_id == 10784 || $goods_id == 10785 )
        {
            if($message_url != "")
            {
                ...
                //echo urlencode($message_url);exit;
            } 
        }
