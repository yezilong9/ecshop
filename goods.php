...
/* add by hg for date 2014-03-26 限制商户或代理商进入不属于自己范围内的商品*/
$message = judge_agency($goods_id);
if($message != true)
{
	ecs_header("Location: ./\n");
	exit;
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    //取出产品详细描述中的图片进行域名地址分割weichen 2015/1/6 10:52:09
    preg_match_all("/\<img.*?src\=\"(.*?)\"[^>]*>/i", $goods['goods_desc'],$image_match);
    if(count($image_match[1]) > 0)
    {
        foreach ($image_match[1] as &$value)
        {
            // loop through values 替换图片的路径
            if(!strstr($value,'http://'))
            {
                $img_url_src = img_url().substr($value, 1);
                $goods['goods_desc'] = str_replace($value, $img_url_src, $goods['goods_desc']);
            }
        } 
    }
	//dump($goods);
    ...
    else
    {
       ...
       $goods['goods_style_name'] = add_style($goods['goods_name'], $goods['goods_name_style']);
		...
		$agency_where = agency_goods();
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id FROM " .$ecs->table('goods'). " WHERE $agency_where cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
        ...
        $next_gid = $db->getOne("SELECT max(goods_id) FROM " . $ecs->table('goods') . " WHERE $agency_where cat_id=".$goods['cat_id']." AND goods_id < ".$goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
        if (!empty($next_gid))
        {
		...
		/*add by hg for date 2014-04-25获取实际属性 begin*/
		$host_goods_id = $db->getOne("select host_goods_id from ".$ecs->table('goods')." where goods_id = $goods_id ");
		if($host_goods_id)
		{
			 $properties = get_goods_properties($host_goods_id);  // 获得商品的规格和属性
		}else{
...
/*二维码信息*/
$codeUrl = agency_url();
$codeUrl = 'http://'.$codeUrl.'/codeImg.php?url=http://'.$codeUrl.'/mobile/goods.php?id='.$goods_id.'&goods_id='.$goods_id;
...
function get_linked_goods($goods_id)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            "WHERE $agency_where lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['market_price'] = price_format($row['market_price'],true,'1');
...
function get_user_rank_prices($goods_id, $shop_price)
{
	...
	if(!$agency_where)
	{
		$agency_where = ' r.admin_agency_id = 0 AND ';
	}
	else
	{
		$agency_where = 'r.'.$agency_where;
	}
    $sql = "SELECT rank_id, IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " .
            'FROM ' . $GLOBALS['ecs']->table('user_rank') . ' AS r ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = '$goods_id' AND mp.user_rank = r.rank_id " .
            "WHERE  $agency_where (r.show_price = 1 OR r.rank_id = '$_SESSION[user_rank]') AND r.rank_id <>4";
    $res = $GLOBALS['db']->query($sql);
...
function get_also_bought($goods_id)
{
    ...
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
            ...
            "WHERE $agency_where a.goods_id = '$goods_id' AND b.goods_id <> '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " .
            'GROUP BY b.goods_id ' .
            'ORDER BY num DESC ' .
...
/*根据用户等级显示对应的起购数量 add by zenghd for date 2014-08-22*/
