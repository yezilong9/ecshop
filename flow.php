 ...
 *  购物流程
 * ============================================================================
 * 版权所有 2005-2010 广州新泛联数码有限公司，并保留所有权利。
 * 网站地址: http://www..com；
 * ----------------------------------------------------------------------------
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    /* 更新：如果是一步购物，先清空购物车 */
    if (($_CFG['one_step_buy'] == '1') || $goods->mooncake)
    {
        clear_cart();
    }
	/*检查起购数量 add by for 2014-06-09*/
	if(!empty($goods->number) && !empty($goods->goods_id))
	{
		//购物车商品数量
		$cart_goods_number = $db->getOne('select goods_number from '.$ecs->table('cart'). " where user_id = $_SESSION[user_id] and goods_id = $goods->goods_id" );
		//该商品的起购数量
		$goods->number+$cart_goods_number;
		$start_goods_num = $db->getOne('select start_num from '.$ecs->table('goods'). " where goods_id = $goods->goods_id" );
		
		/*根据用户等级显示对应的起购数量 add by zenghd for date 2014-08-19*/
		if($_SESSION['user_id']){//用户是否登录
			$start_goods_num = get_user_start_num($_SESSION['user_id'],$goods->goods_id);
		}
		
		if($goods->number+$cart_goods_number <  $start_goods_num)
		{
			$result['error']   = 7;
            $result['message'] = '购买数量低于起购数量';
			die($json->encode($result));
		}
	}
    /* 检查：商品数量是否合法 */
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            /*ccx 2014-12-15 判断输入的六位短信验证码是否正确 开始*/
            $txd_user_code_1 = $_POST['txd_user_code_1'];
            if(empty($_POST['txd_user_code_1']) || strlen($txd_user_code_1) != 6 )
            {
                show_message("请输入正确的六位短信验证码");
            }
            $sql_count_ccx =  "SELECT id FROM ".$ecs->table('txd_user_code').
                              " WHERE account ='".$_POST['username']."' AND code = " .$txd_user_code_1 ." AND status = 0 AND type = 1 ";
            $record_count_ccx = $db->getOne($sql_count_ccx);
            if($record_count_ccx == '')
            {
                show_message("对不起，您输入的6位短信验证码不正确,请检查确认再次输入");
            }  
            /*ccx 2014-12-15 判断输入的六位短信验证码是否正确 结束*/
...
{//print_r($_SESSION);
	...
	/*检查商品起购数量 add by hg for date 2014-06-24 */
	$error_goods_arr = check_goods_start_num();
	if($error_goods_arr)
	{
		show_message('<span style="color:black">'.$error_goods_arr.'</span>购买数量低于起购数量', $_LANG['back_to_cart'], 'flow.php');
	}
    /* 取得购物类型 */
    $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    ...
    /*ccx 2014-12-17 圣诞活动送苹果 获取活动中的留言链接地址,并且要在订单附言中显示这些留言备注(链接地址) 开始*/
    if($_REQUEST['message_url'] != '')
    {
        //$smarty->assign('message_url', $_REQUEST['message_url']);
        $message_url = substr(substr($_REQUEST['message_url'], 0, -2), 2 );
        $smarty->assign('message_url', $message_url);
    }
    /*ccx 2014-12-17 圣诞活动送苹果 结束 */
	...
	/*代理商余额不足不能进行购物的动作 2014-05-22 by hg*/
	check_agecny_remaining($total['goods_price']);
    ...
    // 查看购物车中是否有免运费商品，若是则把运费赋为零 add by zenghd 2014-09-15 20:30*/
    ...
    $sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 1";
    $shipping_count = $db->getOne($sql);
    
    /*如果某种配送方式同时设置了全国的运费和该省份的运费，过滤掉全国的运费，只使用该省份的运费 add by zenghd 2014-09-15 20:30*/
    $s_id = array();
    foreach ($shipping_list AS $s_key => $s_val)
    {
    	if($s_val['region_id'] == $consignee['province']){
    		$s_id[] = $s_val['shipping_id'];
    		
    	}
    }
    foreach ($shipping_list AS $s_key => $s_val)
    {
    	if(in_array($s_val['shipping_id'], $s_id) && ($s_val['region_id'] == 1))
    	{unset($shipping_list[$s_key]);}
    }//过滤结束
    ...
    foreach ($shipping_list AS $key => $val)
    {
		...
		//$shipping_list[$key]['destination'] = $consignee['address'];
        $shipping_list[$key]['format_shipping_fee'] = price_format($shipping_fee, false,true);
        $shipping_list[$key]['shipping_fee']        = $shipping_fee;
        $shipping_list[$key]['free_money']          = price_format($shipping_cfg['free_money'], false,true);
    ...
    {
        // 取得用户可用红包
        /*2014/12/20 18:39:04 weichen 原版不限制红包的代码 圣诞节结束后 恢复
        ...
        //应对圣诞节活动 做出针对产品限制红包
        /*$arr_christmas_goods_id_10 = array(10755,10756,10757,10758,10759,10760,10761,10762,10763,10764,10765,10766,10767,10768,10769,10770,10771,10774,10775,10778,10779,10780,10781,10782,10783,10784,
            10785,10786,10787,10788,10789,10790,10791,10792,10793,10794,10795,10796,10797,10798,10799,10800,10801,10802,10803,10804,10805,10806,10807,9423
        );
        $arr_christmas_goods_id_20_30_50 = array(10779,10780,10781,10782,10783,10784,10785);

        $user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price']);
        if (!empty($user_bonus))
        {
            $user_bonus_used = $user_bonus;
            foreach ($user_bonus AS $key => $val)
            {
                $bonus = false;//默认不显示这个红包
                foreach($cart_goods as $k2=>$v2)
                {
                    //满足10元的产品
                    if(in_array($v2['goods_id'],$arr_christmas_goods_id_10) && $val['type_money'] <= 10)
                    {
                        $bonus = true;
                        break;
                    }
                    //满足50元以下的产品
                    if(in_array($v2['goods_id'],$arr_christmas_goods_id_20_30_50) && $val['type_money'] <= 50)
                    {
                        $bonus = true;
                        break;
                    }
                ...
                if(!$bonus)
                {
                    unset($user_bonus_used[$key]);
                }
                else
                {
                    $user_bonus_used[$key]['bonus_money_formated'] = price_format($val['type_money'], false);
                }
                
            }
            $user_bonus = $user_bonus_used;
            $smarty->assign('bonus_list', $user_bonus);
        }*/     
  
        
        //ccx 2015-03-09 start应对荔枝预售情况 做出针对荔枝的商品限制红包
        $user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price']);
        if (!empty($user_bonus))
        {
            $user_bonus_used = $user_bonus;
            foreach ($user_bonus AS $key => $val)
            {
                ...
                foreach($cart_goods as $k2=>$v2)
                {
                    //if($v2['goods_id'] == 10791 && $val['type_money'] == 30)  //商品的id，以及红包的金额(也可以通过type_id==20来判断)，到时候需要填写服务器上面的数据
                    if($v2['goods_id'] == 10791 && $val['type_id'] == 20)
                    {
                        $bonus = true;
                        break;
                    }
                    //elseif ($val['type_money'] != 30)
                    elseif ($val['type_id'] != 20)
                    {
                       $bonus = true;
                       break; 
                    }
		...
		/* 购物车是否包含有某个商品 月饼活动专用 date 2014-08-19 by hg */
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
        ...
        'order_ip' 	      => real_ip(),		//add By SouthBear 2014-08-20 15:09:12
   ...
   /* 判断是不是实体商品 */
	...
	/*注释修改于2014-04-02 by hg */
	/*if ($order['pay_id'] > 0)
    {
        $payment = payment_info($order['pay_id']);
        $order['pay_name'] = addslashes($payment['pay_name']);
    } */
	if($order['pay_num'] != '1' && $order['pay_num'] != '2')
	{
		//注释修改于2014-11-11 by ccx  数据读取的表应该是原有ecshop的ecs_payment的支付方式表！
		//$psy_res = $db->getRow("select bank_name from ". $ecs->table('pay_bank'). " where bank_code = '$order[pay_num]'");
		//$order['pay_name'] = $psy_res['bank_name'];
		
		
		/* ccx 根据他选择的银行, 判断他最后选择的支付方式 */
		$psy_res = $db->getRow("select pay_id, bank_name from ". $ecs->table('pay_bank'). " where bank_code = '$order[pay_num]'");
		$order['pay_name'] = $psy_res['bank_name'];
		$order['pay_id'] = $psy_res['pay_id'];		
		
	}
	elseif($order['pay_num'] == '1')
	{
		$order['pay_name'] = '余额支付';
		/*注释修改于2014-11-11 by ccx 
		记录订单当中的支付方式的pay_id, 如果是1 就是余额支付，如果是2就是货到付款，其他的就是在线支付，在线支付分为很多银行那种支付*/	
		$order['pay_id'] = $order['pay_num'];
	}
	elseif($order['pay_num'] == '2')
	{
		$order['pay_name'] = '货到付款'; 
		$order['pay_id'] = $order['pay_num'];
	}
	...
	$payment=array();
	/*end*/
    if ($order['pay_num'] == '1' && $order['order_amount'] > 0)
    {
    ...
    /*ccx 2014-12-15 时间限定，在规定的时间内，只要下了订单就会产生相应的活动代码 本地活动到2014-12-31 开始*/
     if(gmtime() < 1420041600 ) // 2014-12-31 24:00:00 时间戳就是1420041600 
     {
        $order['activity_code'] ='2014120001';  //活动代码表示:2014年12月加上后面0001四位
     }
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                "order_id, costing_price,goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) ".
            " SELECT '$new_order_id', costing_price,goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id".
            " FROM " .$ecs->table('cart') .
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
		...
		{
			agency_order_log($order['order_id'],$order['admin_agency_id'],$order['user_id'],$order['goods_amount']);
		}
    }
    ...
    if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
    {
       // include_once('includes/cls_sms.php');
		...
		/*add by zenghd 2014-9-28 发送短信的方式*/
		$obj_sms = new class_sms($_CFG['sms_shop_mobile'],sprintf($msg, $order['consignee'], $order['tel']));
		...
		if ($virtual_goods AND $flow_type != CART_GROUP_BUY_GOODS)
        {
            ...
            if (virtual_goods_ship($virtual_goods,$msg, $order['order_sn'], true))
            {
                ...
                $sql = "SELECT COUNT(*)" .
                        ...
                        " AND is_real = 1";
                if ($db->getOne($sql) > 0)//$db->getOne($sql) <= 0
                {
                    /* 修改订单状态 */
                    ...
                    if ($order['user_id'] > 0)
                    {
                        ...
                        /* 发放红包 2014/12/22 18:03:12 圣诞活动结束后在开启 weichen */
                        //send_order_bonus($order['order_id']);
                    }
                }
            }
    ...
    /* ccx 2014-12-14 判断如果是货到付款的情况，也有抽奖机会  开始*/
    elseif($order['order_amount'] <= 0 && $order['pay_num'] == 1 )  
    {
        
        $sign_message = 'order_id='.$order['order_id'].'&payprice='.$order['order_amount'].'&email=0&user_id='.
...
//    die(sprintf($_LANG['bonus_is_ok'], price_format($bonus['type_money'], false)));
    ...
    else
    {
        ...
        if($total['goods_price']<$bonus['min_goods_amount'])
        {
         ...
         /* 重新计算订单 */
         $total = order_fee($order, $cart_goods, $consignee);
         $result['error'] = sprintf($_LANG['bonus_min_amount_error'], price_format($bonus['min_goods_amount'], false,true));
        }
...
elseif($_REQUEST['step'] == 'ajax_check_start_goodst')
{
	include_once('includes/cls_json.php');
	$json  = new JSON;
	$res = check_goods_start_num();
	$result = $res?'0':'1';
	die($json->encode($result));
}
else
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr AS $key => $val)
    {
		...
		/*起购数量 add by hg for 2014-06-09*/
		$cart_goods_id = $GLOBALS['db']->getOne("select goods_id from ".$GLOBALS['ecs']->table('cart')." where rec_id = $key");
		$goods_start_num = $GLOBALS['db']->getOne("select start_num from ".$GLOBALS['ecs']->table('goods')." where goods_id = $cart_goods_id");
		
		/*根据用户等级显示对应的起购数量 add by zenghd for date 2014-08-19*/
		if($_SESSION['user_id']){//用户是否登录
			$goods_start_num=get_user_start_num($_SESSION['user_id'],$cart_goods_id);
		}
		
		if($val < $goods_start_num)
		{
			show_message('更新失败,购买数量低于起购数量', $_LANG['back_to_cart'], 'flow.php');
		}
...
/*根据用户等级显示对应的起购数量 add by zenghd for date 2014-08-19*/
function get_user_start_num($user_id,$goods_id){
	$sql_user_rank = "SELECT user_rank FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id= '".$user_id."'";
	$user_rank = $GLOBALS['db']->getOne($sql_user_rank);
	if($user_rank>0){//用户等级是否存在
		$sql_rank_goods_start_num = "SELECT rank_goods_start_num FROM " .$GLOBALS['ecs']->table('member_price'). " WHERE user_rank= '$user_rank' AND goods_id= '$goods_id' ";
		$rank_goods_start_num = $GLOBALS['db']->getOne($sql_rank_goods_start_num);//对应等级与对应商品的起购数量
		
		if($rank_goods_start_num){
			$user_start_num = $rank_goods_start_num;
		
		}else{
			$sql_rank_start_num = "SELECT rank_start_num FROM " .$GLOBALS['ecs']->table('user_rank'). " WHERE rank_id= ' $user_rank'";
			$rank_start_num = $GLOBALS['db']->getOne($sql_rank_start_num);//等级默认的起购数量
			
			if($rank_start_num){
				$user_start_num = $rank_start_num;
			
			}else{
				
				$goods_start_num = $GLOBALS['db']->getOne('select start_num from '.$GLOBALS['ecs']->table('goods'). " where goods_id = $goods_id" );
				$user_start_num = $goods_start_num;
			}
		}
   }else{
		$goods_start_num = $GLOBALS['db']->getOne('select start_num from '.$GLOBALS['ecs']->table('goods'). " where goods_id = $goods_id " );
 ...
 return $user_start_num;
}


/* ccx 2015-04-20 satrt */
function edit_parent_id($user_id, $admin_agency_id)
{
    //echo "<br>"; echo $user_id; echo $admin_agency_id; echo "<br>";
    $sql_parent_id = "SELECT parent_id, top_rank, user_rank FROM " .
                      $GLOBALS['ecs']->table('users'). 
                      " WHERE user_id= ".$user_id ;
    $parent_num = $GLOBALS['db']->getRow($sql_parent_id);
    $parent_id   = $parent_num['parent_id'];
    $top_rank    = $parent_num['top_rank'];
    $user_rank   = $parent_num['user_rank'];
    if($admin_agency_id != 0) // 在代理商站点
    {
        if($parent_id ==0 && $top_rank ==0 && $user_rank == 0)  //没有推荐人，没有等级，没有归属代理商
        {
            $sql_up = " UPDATE " . $GLOBALS['ecs']->table('users').
                       " SET parent_id = ". $admin_agency_id . ", top_rank = ". $admin_agency_id . 
                       " WHERE user_id = ". $user_id;
