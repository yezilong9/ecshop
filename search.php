...
if ($_REQUEST['act'] == 'advanced_search')
{
	...
	$smarty->assign('hot_goods',       $arr);// 热卖商品
...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $db->autoReplace($ecs->table('keywords'), array('date' => local_date('Y-m-d'),
                'searchengine' => '', 'keyword' => addslashes(str_replace('%', '', $val)), 'count' => 1), array('count' => 1));
        }
        $keywords .= ')';
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE $agency_where g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] = preg_replace("|$_REQUEST[keywords]|",'<span style="color:red;">'.
		$_REQUEST['keywords'].'</span>',$arr[$row['goods_id']]['goods_name']);
        ...
        $arr[$row['goods_id']]['market_price']  = price_format($row['market_price'],true,true);
        $arr[$row['goods_id']]['shop_price']    = !preg_match('/￥/',price_format($row['shop_price']))?
        ...
        $arr[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? 
		(!preg_match('/￥/',price_format($promote_price))?
			'<span style="font-size:12px;text-decoration:line-through;font-weight:normal;color:#666666;">市场价:'.price_format($row['market_price'],true,true).'</span>'
			:price_format($row['shop_price'])) : '';
	...
	//dump($arr);
	if($_REQUEST['keywords']) $smarty->assign('search_keywords',$_REQUEST['keywords']);
	if(array_filter($arr)) $smarty->assign('search','search');
	...
	//dump($_CFG);
	$smarty->assign('hot_goods',       $array);// 热卖商品
    $smarty->assign('promotion_info', get_promotion_info());
