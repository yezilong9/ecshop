...
elseif ($_REQUEST['act'] == 'view')
{
	...
	$agency_id = agency_id();
	$admin_agency_id = $db->getOne("SELECT admin_agency_id FROM ".$ecs->table('goods_activity')." WHERE act_id = $group_buy_id");
    if (($group_buy_id <= 0) || ($admin_agency_id != $agency_id))
    {
        ecs_header("Location: ./\n");
        exit;
    }
	...
	$group_buy['difference'] = $group_buy['price_ladder'][0]['amount'] - $group_buy['valid_goods'];
    if (empty($group_buy))
    {
        ecs_header("Location: ./\n");
        exit;
...
elseif ($_REQUEST['act'] == 'buy')
{
	...
	$user_nem = user_buy_num($_SESSION['user_id'],$group_buy_id);

    /* 查询：取得团购活动信息 */
    $group_buy = group_buy_info($group_buy_id, $number);
	if(($number+$user_nem) > $group_buy['oncenumber'])
	{
		 show_message('购买数量大于单人购买数量', '', '', 'error');
	}
...
{
	$where = substr(' AND b.'.agency_goods(),0,-3);
	
    /* 取得团购活动 */
    $gb_list = array();
    $now = gmtime();
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb,g.original_img, b.act_id AS group_buy_id, g.market_price, ".
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
		...
		$group_buy['price'] = $group_buy['price_ladder'][0]['price']; //现价
	...
	/* 排序 */
	$gb_list = gd_list_order($gb_list);
    return $gb_list;
}
/* 用户团购数量 */
function user_buy_num($user_id,$group_buy_id)
{
	if(empty($user_id) || empty($group_buy_id)) return false;
	$res = $GLOBALS['db']->getAll("SELECT g.goods_number FROM ".$GLOBALS['ecs']->table('order_info')." AS o LEFT JOIN ".
	$GLOBALS['ecs']->table('order_goods')." AS g ON o.order_id = g.order_id WHERE o.extension_id = $group_buy_id");
	$number = 0;
...
/* 数组排序 */
function gd_list_order($gb_list)
{
	$order_name = isset($_GET['order_name'])?$_GET['order_name']:'';
	$oder       = isset($_GET['oder'])?$_GET['oder']:'';
	if($order_name && $oder){
		if($order_name == 'cur_amount'){
			if($oder == 'desc'){
				usort($gb_list,"desc_cur_amount");//自定义排序
			}else{
				usort($gb_list,"asc_cur_amount");//自定义排序
			}
			//赋值
			$GLOBALS['smarty']->assign('cur_amount',$oder=='desc'?'asc':'desc');
		}elseif($order_name == 'price'){
			if($oder == 'desc'){
				usort($gb_list,"desc_price");//自定义排序
			}else{
				usort($gb_list,"asc_price");//自定义排序
			}
			//赋值
			$GLOBALS['smarty']->assign('price',$oder=='desc'?'asc':'desc');
		}elseif($order_name == 'end_time'){
			if($oder == 'desc'){
				usort($gb_list,"desc_end_time");//自定义排序
			}else{
				usort($gb_list,"asc_end_time");//自定义排序
			}
			//赋值
			$GLOBALS['smarty']->assign('end_time',$oder=='desc'?'asc':'desc');
		}
...
/* 售量倒序排序 */
function desc_cur_amount($x,$y)
{ 
	global $order_name;
	if($x['cur_amount'] == $y['cur_amount']) 
		return 0; 
	elseif($x['cur_amount'] < $y['cur_amount']) 
		return -1; 
	else 
		return 1; 
}
/* 售量正排序 */
function asc_cur_amount($x,$y)
{ 
	global $order_name;
	if($x['cur_amount'] == $y['cur_amount']) 
		return 0; 
...
/* 时间倒序排序 */
function desc_end_time($x,$y)
{ 
	global $order_name;
	if($x['end_time'] == $y['end_time']) 
		return 0; 
	elseif($x['end_time'] < $y['end_time']) 
		return -1; 
	else 
		return 1; 
} 
/* 时间正排序 */
function asc_end_time($x,$y)
{ 
	global $order_name;
	if($x['end_time'] == $y['end_time']) 
		return 0; 
